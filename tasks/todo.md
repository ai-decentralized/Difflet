...
T2V serving architecture research (2026-07-13):
- [x] Record branch/repository anchor and trace the existing T2I serving lifecycle.
- [x] Map Wan, HunyuanVideo, HunyuanVideo 1.5, and LTX-2 CLI stages and artifacts.
- [x] Assess single-process co-residency from topology, artifact, and measured memory evidence.
- [x] Write the Phase-1 T2V serving summary, architecture, API, usage, and adaptation docs.
- [x] Verify documentation links, formatting, and `git diff --check`.
- Rationale: reuse the generic one-process stage engine, but gate every T2V model
  on uniform Neuron topology and measured co-residency instead of assuming that
  CLI subprocess artifacts can be loaded together.
- Verification: all relative Markdown targets exist; `feature/serving_t2v` and
  `feature/serving_t2i` share base `f75e13b`; `git diff --check` passes.
...
Image serving response follow-up (2026-07-14):
- [x] Keep `/v1/chat/completions` and return a Base64 data URL when R2 is unconfigured.
- [x] Auto-select R2 only for a complete required configuration; reject partial configuration.
- [x] Move the serve command implementation from `difflet/serving/cli/` to `difflet/cli/`.
- [x] Update README and authoritative serving docs for optional R2 and the vLLM-Omni chat response contract.
- [x] Add focused storage/response/CLI tests and run formatting, lint, compile, and unit verification.
- Rationale: keep one stable Chat Completions response shape while making object
  storage a deployment option rather than a prerequisite.
- Verification: `tests/unit/serving` passed 216 tests; focused CLI tests passed
  47 tests; Black, Ruff, targeted mypy with skipped imports, `compileall`, and
  `git diff --check` passed.

T2V Videos API implementation (2026-07-16):
- [x] Freeze the provisional single-model residency assumption, current CPU
  placement findings, functional blockers, and Trainium investigation register.
- [x] Complete independent plan/API/current-serving review before implementation.
- [x] Extend runtime contracts for honest host/neuron/hybrid stage placement and
  file-backed generated output without changing image positional contracts.
- [x] Record the user's lifecycle decision: match vLLM-Omni with process-local
  in-memory video jobs; stopping/restarting Serving discards every job and old
  IDs return `404` while list starts empty.
- [x] Implement multipart Videos protocol normalization, transition-checked
  `InMemoryVideoJobRepository`, confined local MP4 store, PyAV
  encode/validation, and lifecycle cleanup.
- [x] Keep the same-profile media-root lease and file-backed atomic publication;
  clean shutdown purges jobs and all managed MP4 files, while the next lease
  holder purges residue left by a crashed process before accepting requests.
- [x] Implement one live-capacity-bounded FIFO `VideoGenerationService` shared
  by Videos sync and async requests, including cancellation/recovery fencing.
- [x] Implement all six `/v1/videos` routes with a fake resident engine and keep
  the image Chat Completions contract unchanged.
- [x] Add provisional resident adapters/metadata for LTX-2, Wan 2.1, and
  HunyuanVideo 1.0 using current lower-layer primitives only.
- [x] Keep Wan 2.2 and HunyuanVideo 1.5 outside the HTTP Serving allowlist with
  distinct reasons: Wan 2.2 has a successful real Trn2 single-transformer
  offline benchmark but current wiring disables `transformer_2`;
  HunyuanVideo 1.5 offline compile/generate is still a scaffold.
- [x] Run focused/full Serving tests, formatting/type/compile/diff checks, an
  independent final review, and prepare the per-model Trainium validation packet.
- [x] Record the stronger application-wide admission decision: every FastAPI
  generation endpoint targeting one resident engine, including Chat
  Completions and Videos sync/async, shares one global ticket FIFO, capacity,
  deadline, and recovery domain; reads bypass it and DELETE uses the queued
  deletion control plane.
- [ ] Generalize the Videos-only service into that global admission owner,
  route Chat through it, define FIFO at monotonic ticket assignment, disable
  every inner waiting queue, and keep the physical queue bounded after queued
  deletion/disconnect.
- [ ] Add mixed Chat/Video order and capacity tests, a single-engine-caller
  assertion, unified deadline/recovery tests, and a repeated enqueue/delete
  test that proves physical queue memory remains bounded.
- [x] Preload every image/video CPU tokenizer before readiness and move
  per-request tokenization/model validation to a bounded executor before ticket
  assignment.
- [x] Bound validation admission itself with defaults of 4 CPU workers/32 waiting,
  a 30-second sub-deadline, stable 429/504 errors, and total-deadline accounting
  that starts before validation.
- [x] Keep validation and generation concurrency distinct: at most four CPU
  validation futures but exactly one model generation. After validation
  timeout/disconnect, retain a started slot until its future ends; release an
  unstarted slot only after atomic physical removal. Test repeated races against
  the 4/32 bounds.
- [x] Enforce the T2V P0 multipart contract while reading the body: zero files,
  at most 32 fields, 256 KiB per text part, and 1 MiB total including chunked
  input; return stable 413 on overflow.
- [x] Narrow public DELETE to queued work: remove queued tickets, return
  `409 video_in_progress` after dispatch, and reserve terminate/restart fencing
  for internal deadline/shutdown/worker-failure recovery.
- [x] Add process-local terminal retention: `expires_at` at 25 hours, periodic
  sweeper, structured disk-pressure warning/error logs, and hard storage-reserve
  rejection with `507 video_storage_full`.
- [x] Add the 4,096-record async-job cap, cumulative sync/async reservation
  ledger, explicit release paths, and artifact-lifecycle serialization for
  publication/content lease/terminal DELETE/sweeper.
- [x] Make queued DELETE and dispatcher claim one admission-lock linearization
  point; reuse it for sync disconnect so successful queued deletion can never
  execute later.
- [ ] Execute the real `trn2.3xlarge` release gate for LTX-2, Wan 2.1, and
  HunyuanVideo 1.0: exact artifact compile/load, startup peak and steady-state
  RSS/PSS/HBM, first and second generation, all six API flows, timeout/recovery,
  and a repeated-request soak for every fixed profile.
  - [x] LTX-2 TP4/CP1 480x704x49: cold download/compile, resident load,
    two-step startup smoke, health/readiness, repeated sync/async generation,
    all six routes, queued DELETE/running rejection, media validation,
    five-second RSS/PSS/host/HBM sampling, graceful shutdown, and HBM release.
  - [ ] LTX-2 timeout/recovery and long repeated-request soak.
  - [ ] Wan 2.1 resident gate.
    - [x] Existing owned service: resident load, HBM/RSS/PSS phases, sync/async,
      all six API routes, MP4 validation, running and queued DELETE semantics.
    - [ ] Owned clean shutdown/HBM-release run after the current service exits.
  - [ ] HunyuanVideo 1.0 resident gate.
- [x] Audit `benchmark/trn2` and record the corrected evidence boundary:
  LTX-2, Wan 2.1, and HunyuanVideo 1.0 have successful real Trn2 fixed-profile
  offline generation; Wan 2.2 has only the current single-transformer path;
  HunyuanVideo 1.5 remains pending.
- [x] Separate offline evidence from resident Serving qualification: benchmark
  warm samples reload weights in separate CLI processes and retain finite
  `.pt` tensors. LTX-2 now has a separate measured resident run; Wan and
  HunyuanVideo peak HBM/RSS/PSS plus six-API flows remain unmeasured.
- [ ] Align the Wan 2.2 benchmark label with its recorded single-transformer
  execution and distinguish superseded HunyuanVideo timing notes from current
  summary values.
- Rationale: memory fit is no longer a coding gate, but correctness, compatible
  process worlds, cancellation-safe publication, and media validity remain
  mandatory. One instance continues to serve one fixed model/profile.
- Local verification: the updated Serving suite passed 436 tests with 5 skipped;
  formatting, changed/new-file Ruff, targeted mypy, compile/import, relevant
  CLI, and diff checks passed. Independent final review found no remaining P0/P1
  issue within the completed Videos endpoint milestone. The subsequently added
  global Chat/Videos admission requirement remains an explicit implementation
  item. Real T2V Trn2 offline benchmark evidence exists and is recorded in
  `docs/design/t2v_serving/07_trn2_benchmark_evidence.md`. A separate 2026-07-17
  LTX-2 run on `16.27.26.203` passed the measured fixed-profile resident/API
  subset and is recorded in
  `docs/design/t2v_serving/08_ltx2_trn2_serving_validation.md`; timeout/recovery,
  soak, Wan, and HunyuanVideo gates remain open.

T2V serving phase-resource capture (2026-07-17):
- [x] Add a reusable process-tree and raw `neuron-monitor` JSONL sampler.
- [x] Capture compile tail, pre-shard, artifact publish, worker load, startup
  smoke, idle, three generations/queue cancellation, and shutdown on Trn2.
- [x] Preserve all 302 five-second samples, serve/API evidence, and validated
  MP4 outputs under `artifacts/remote-logs/16.27.26.203/t2v/ltx2/`.
- [x] Document phase peaks, accounting caveats, per-core HBM, and implications
  for future compatible-stage co-residency in English and Chinese.
- Rationale: ordinary RSS/PSS alone omits Neuron runtime host allocation and
  HBM; retaining both raw domains plus system `MemAvailable` avoids an unsafe
  co-residency conclusion.
- Verification: the sampler produced 302 valid JSON records at five-second
  cadence. The first 26 compile records preserve a handled initial monitor
  timeout and the remaining 276 contain valid monitor payloads. It captured
  38.957 GiB steady HBM (9.739 GiB per core), 31.10 GiB idle Neuron host
  allocation, 46.86 GiB pre-shard PSS peak, zero swap, and three zero-runtime
  post-shutdown samples. Black, Ruff, and `py_compile` pass for
  `scripts/sample_serving_resources.py`.

Wan 2.1 Trn2 resident/API validation (2026-07-17):
- [x] Reuse the existing ready TP4/CP1 Wan 2.1 service without colliding with
  or stopping its owner process.
- [x] Preserve 410 startup samples and 79 independent API-phase samples,
  including HBM 41.343 GiB total / 10.336 GiB per core and MP4/API evidence.
- [ ] Repeat Wan 2.1 with owned process shutdown to prove runtime/HBM release.
- Verification: sync 57.583 s, async 57.443 s, both PyAV-validated as
  832x480/16 fps/9 frames; running DELETE returned `409 video_in_progress`,
  queued DELETE removed the job and subsequent GET returned 404; all six routes
  completed successfully. The existing service remains running on the remote
  host because this validation did not own its lifecycle.

T2V Serving design Chinese translation (2026-07-16):
- [x] Translate the complete eight-document T2V Serving design set while
  preserving Markdown structure, Mermaid diagrams, API paths, code identifiers,
  and status boundaries.
- [x] Point cross-document links at the Chinese counterparts and verify every
  local target.
- [x] Keep the English originals unchanged and normalize “provisional” as
  “暂定” where it describes an assumption, adapter, target, or threshold.
- Verification: English/Chinese heading, code-fence, Mermaid, and endpoint-token
  counts match; all Chinese relative links resolve; whitespace and
  `git diff --check` pass.

T2V Serving iterative design review (2026-07-16):
- [x] Complete local self-review and run the Serving suite (`422 passed, 5 skipped`).
- [x] Run fallback Codex reviewer Round 1 and validate its findings against code.
- [x] Record Round 1 in `docs/design/t2v_serving/review.md` without changing the design.
- [x] Apply only user-confirmed Round 1 feedback to the English design and Chinese mirrors.
- [x] Run same-reviewer Round 2, present its four material findings, obtain user
  acceptance, and apply them to the English design and Chinese mirrors.
- [x] Run same-reviewer Round 3, accept its validation-timeout ownership finding,
  distinguish CPU validation from model generation, and apply the
  English/Chinese deadline and capacity fix.
- [x] Reuse the same reviewer through Round 4 convergence with no
  blocking/material findings.
- [ ] Run one fresh independent fallback review before declaring convergence.

Original video CLI walkthrough (2026-07-16):
- [x] Locate the public CLI entry, validation, registry dispatch, and stage subprocess boundary.
- [x] Trace Wan, HunyuanVideo, and LTX-2 from `generate` through MP4 export and cleanup.
- [x] Compare the current worktree with `HEAD` so serving changes are not mistaken for the original batch CLI.
- [x] Run parser/help and focused CPU-safe CLI tests; record Trainium-only gaps.
- [x] Write a concise Chinese source-reading and operational walkthrough.
- Rationale: keep the original offline batch path distinct from the uncommitted
  resident T2V serving work, and expose correctness/reproducibility hazards before
  using the CLI as a serving baseline.
- Verification: CLI root/generate help passed; 80 CPU-safe CLI parsing,
  validation, stage-wiring, and request-loop tests passed; targeted compileall
  and `git diff --check` passed. Real tensor/MP4 generation was not run locally
  because this host lacks torch/numpy and Trainium; existing Trn2 evidence is
  linked without being presented as fresh validation.
