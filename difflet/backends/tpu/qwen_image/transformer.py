...
class TpuQwenImageTransformerApplication(TpuApplicationBase):
    ...
    def _materialize_meta_(module: torch.nn.Module, device="cpu", dtype=None) -> None:
        ...
        NOT ``Module.to_empty()``: that blanks buffers as well, and the static
        RoPE (``img_cos``/``img_sin``) is a buffer computed for real during
        ``__init__`` — precisely what ``init_empty_weights(include_buffers=False)``
        went out of its way to keep. Blanking it yields uninitialized rotary
        embeddings, which corrupts every image without raising anything.
        ...
        converting the *incoming* tensor just upcasts it straight back, giving
        20.4 GiB per rank instead of 9.5 GiB. Buffers keep their computed
        dtype — the rope math runs in fp32 internally.
        """
