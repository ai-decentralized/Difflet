"""Neuron multi-entry application for the H1c resident cache-step graph."""
...
class _CacheStepInstance(BaseModelInstance):
    ...
    def get(self, bucket_rank, **kwargs):
        ...
        # Outputs 2..4 write back K=2 anchors and the current latent.
        return self.module, {
...
class _CacheStepWrapper(ModelWrapper):
    ...
    def get_model_instance(self):
        ...
        return _CacheStepInstance(_create)


def _full_shape(config) -> tuple[int, int, int]:
    return (1, _seq_len(config), int(config.in_channels))


def _initialize_inputs(config):
    return (
        torch.zeros(_full_shape(config), dtype=config.neuron_config.torch_dtype),
...
class NeuronFluxResidentCacheStepApplication(NeuronApplicationBase):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        definitions = (
            ...
            ("cache_skip_step", ResidentCacheSkipStepModel, _skip_inputs),
...
__all__ = ["NeuronFluxResidentCacheStepApplication"]
