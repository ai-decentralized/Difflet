...
# set DIFFLET_LTX2_NO_WEIGHT_CACHE=1 to disable on memory-constrained hosts). This removes the
...
def _load_block_state_dict_from_dir(
    ...
    *,
    ...
    _cache_on = os.environ.get("DIFFLET_LTX2_NO_WEIGHT_CACHE") != "1"
...
# DIFFLET_LTX2_MX_ALL_E4M3=1 so the existing compile probe can toggle it without
...
def _mx_native_weight_pack_enabled() -> bool:
    return os.environ.get("DIFFLET_LTX2_MX_NATIVE_WEIGHT_PACK", "") == "1"
...
class MXLinear(nn.Module):
    """nn.Linear whose matmul runs through difflet.ops.mx.linear_mx (E4M3).
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        from difflet.backends.trainium.ops_impl.mx import (
            linear_mx_prequant,
        ...
        hoist_activation = os.environ.get("DIFFLET_LTX2_MX_HOIST_ACTIVATION", "") == "1"
...
class _LTX2MXGroupedAttnProcessor:
    ...
    def _project_kv(
        ...
        if (
            os.environ.get("DIFFLET_LTX2_MX_GROUP_KV", "") == "1"
...
def _maybe_swap_block_to_mx(block: nn.Module) -> nn.Module:
    if os.environ.get("DIFFLET_LTX2_MX_ALL_E4M3", "") != "1":
        return block
    _MX_SWAP_REPORT.clear()
    _swap_linears_to_mx(block)
    if os.environ.get("DIFFLET_LTX2_MX_GROUP_KV", "") == "1":
    ...
    report_path = os.environ.get("DIFFLET_LTX2_MX_SWAP_REPORT")
    if report_path:
...
class LTX2BlockSegmentApplication(NeuronApplicationBase):
    ...
    def reload_block_weights(self, block_index: int) -> None:
        ...
        #  (2) OPTIONAL amortized resident reload (DIFFLET_LTX2_RELOAD_TEARDOWN_EVERY, default 1 = off):
        ...
        teardown_every = int(os.environ.get("DIFFLET_LTX2_RELOAD_TEARDOWN_EVERY", "1"))
        ...
        with torch.no_grad():
            ...
            if self.block_load_mode == "process":
                ...
                process_work_dir = Path(
                    os.environ.get(
                        "DIFFLET_LTX2_PROCESS_WORK_DIR",
                        f"/tmp/difflet_ltx2_process_runtime_{os.getpid()}",
                    )
                )
