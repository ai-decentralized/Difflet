...
# Original path: neuronx_distributed_inference/modules/attention/attention_base.py
...
# Difflet fork: KV cache is LLM-only (autoregressive token generation). Diffusion
# does not use KV cache. Stubbed; LLM-only code paths in this file (guarded
# by `is_for_speculation` / `is_for_context_encoding` etc.) become inert.
KVCacheManager = None  # type: ignore[assignment]
KV_CACHE_PAD_FOR_SEQ_IDS_MASKING = -1  # constant; safe default
# Original: from neuronx_distributed_inference.modules.kvcache.kv_cache_manager import KVCacheManager, KV_CACHE_PAD_FOR_SEQ_IDS_MASKING
from difflet.backends.trainium.core.modules.attention.sink import LearnedSink

from difflet.backends.trainium.core.modules.attention.attention_process_groups import (
...
from difflet.backends.trainium.utils.distributed import (
    split_along_dim,
