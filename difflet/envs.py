"""Centralized environment variable registry for Difflet.

Inspired by `vllm/envs.py` (via FastVideo). Every environment variable that
Difflet reads from production code is declared here with a typed parser. Code
should access values lazily as ``difflet.envs.DIFFLET_BACKEND`` etc. — never via
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # Local world size on the node. Set by Difflet modeling code before NxD
    ...
    # Override target platform string surfaced to the compiler. Difflet sets
