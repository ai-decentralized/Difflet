...
**Timestamp:** 2026-09-17 21:14 UTC

> Best-performing configuration: tp=4 + TeaCache calibrated adaptive (--teacache-speedup at cadence 2's skip budget, --teacache-calibration per model); tp=4 (registry default tp=8 -> 4 on trn2.3xlarge), bf16, attention_cte
...
| **e2e generate — cold start** (page cache dropped) | **5.2 min (310 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 4.6 min (273 s) |
| **e2e generate — warm cache** | **40.86 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 25.01 s |

> Cold vs warm: **5.2 min (310 s) → 40.86 s** (7.6× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 274.4 ms | 274.5 ms | 275.4 ms | 272.1 ms | 18 |
| end-to-end (warm) | 40.86 s | 40.86 s | 40.86 s | 40.86 s | 1 |

**Throughput:** 3.645 DiT steps/s
...
|---|---:|---:|
| stage_0 | — | 74.36 s |
| stage_1 | — | 3.1 min (189 s) |
| stage_2 | — | 1.96 s |
| stage_3 | — | 970.0 ms |
| vae_decoder | — | 7.38 s |
| **weights load total** | 0.0 ms | **4.6 min (273 s)** |

- **weights load total:** 4.6 min (273 s) of 5.2 min (310 s) wall
- **compute + overhead (residual):** 36.95 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved flux_1_dev_tp4tcad_out.png |
...
- per-step = 274.4 ms/DiT-step (median 274.5, p90 275.4, n=18) — measured the SAME way as H100: inter-step deltas of a real 28-step generate (wrapping NeuronFluxBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 19 DiT calls timed; warm generate 6s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 310 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 41 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 310 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache calibrated adaptive (--teacache-speedup at cadence 2's skip budget, --teacache-calibration per model); tp=4 (registry default tp=8 -> 4 on trn2.3xlarge), bf16, attention_cte |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model flux_1_dev --config tp4tcad    # true cold + warm e2e
...
#   python -m benchmark.bench --backend cuda --model flux_1_dev --config tp4tcad   # diffusers CUDA reference adapter
```
