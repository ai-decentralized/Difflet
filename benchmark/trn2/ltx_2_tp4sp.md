...
> Best-performing configuration: tp=4 + sequence parallel; tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF)
...
- N/A by design (tp4sp): LTX-2 has no sequence-parallel path (registry supports_sp=False; `difflet` rejects --sp for it, cli/main.py _validate_sp)
...
| best-perf knobs | tp=4 + sequence parallel; tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF) |
...
difflet compile  --model-id Lightricks/LTX-2 --revision 47da56e2ad66ce4125a9922b4a8826bf407f9d0a \
    --tp-degree 4 --cp-degree 1 --sp --height 480 --width 704 --num-frames 49
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model ltx_2 --config tp4sp    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model ltx_2 --config tp4sp    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model ltx_2 --config tp4sp   # diffusers CUDA reference adapter
```
