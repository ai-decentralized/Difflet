...
**Timestamp:** 2026-09-18 10:51 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.6; alpha sweep); tp=4, bf16, joint attention via attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 17.25 s |
| **e2e generate — cold start** (page cache dropped) | **8.4 min (503 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 7.6 min (457 s) |
| **e2e generate — warm cache** | **63.01 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 35.27 s |

> Cold vs warm: **8.4 min (503 s) → 63.01 s** (8.0× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 417.8 ms | 417.6 ms | 418.0 ms | 417.0 ms | 14 |
| end-to-end (warm) | 63.01 s | 63.01 s | 63.01 s | 63.01 s | 1 |

**Throughput:** 2.394 DiT steps/s

## Compile breakdown

| component | build time |
|---|---|
| wall_total_s | 17.25 s |
...
| transformer (denoise loop) | — | 5.4 min (326 s) |
| vae_decoder | — | 8.46 s |
| **weights load total** | 0.0 ms | **7.6 min (457 s)** |

- **weights load total:** 7.6 min (457 s) of 8.4 min (503 s) wall
- **compute + overhead (residual):** 46.05 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved qwen_image_tp4tcod06_out.png |
...
- per-step = 417.8 ms/DiT-step (median 417.6, p90 418.0, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronQwenImageTransformerApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 25s; output finite=True.
...
- e2e_warm = 63 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 503 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.6; alpha sweep); tp=4, bf16, joint attention via attention_cte |
...
difflet generate --model-id Qwen/Qwen-Image --revision 75e0b4be04f60ec59a75f475837eced720f823b6 \
    --tp-degree 4 --cp-degree 1 --height 1024 --width 1024 \
    --steps 20 --guidance-scale 4.0 --seed 42 --teacache-online-delta 0.6 \
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model qwen_image --config tp4tcod06    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model qwen_image --config tp4tcod06    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model qwen_image --config tp4tcod06   # diffusers CUDA reference adapter
```
