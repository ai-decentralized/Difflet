...
**Timestamp:** 2026-09-12 22:38 UTC

> Best-performing configuration: tp=4, --attention-impl sdpa (PyTorch SDPA via XLA instead of attention_cte); tp=4 (registry default tp=8 -> 4 on trn2.3xlarge), bf16, attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 15.8 min (948 s) |
| **e2e generate — cold start** (page cache dropped) | **5.3 min (317 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 4.4 min (266 s) |
| **e2e generate — warm cache** | **50.37 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 20.23 s |

> Cold vs warm: **5.3 min (317 s) → 50.37 s** (6.3× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 682.3 ms | 682.0 ms | 682.3 ms | 681.8 ms | 27 |
| end-to-end (warm) | 50.37 s | 50.37 s | 50.37 s | 50.37 s | 1 |

**Throughput:** 1.466 DiT steps/s
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder_clip | 294.0 ms | 515.0 ms | — | 20.34 s | 96.0 ms | **21.24 s** |
| text_encoder_t5 | 376.0 ms | 730.0 ms | — | 5.98 s | 81.0 ms | **7.17 s** |
| transformer | 1.85 s | 2.65 s | 98.42 s | 3.0 ms | 88.19 s | **3.2 min (191 s)** |
| vae_decoder | 218.0 ms | 209.0 ms | — | 12.0 min (719 s) | 306.0 ms | **12.0 min (720 s)** |
| **Σ component builds** | | | | | | **15.7 min (939 s)** |

> The headline **compile = 15.8 min (948 s)** is the full `difflet compile` wall; the **Σ component builds = 15.7 min (939 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
|---|---:|---:|
| text_encoder_t5 | — | 72.85 s |
| transformer (denoise loop) | — | 3.1 min (185 s) |
| text_encoder_clip | — | 960.0 ms |
| vae_decoder | — | 6.97 s |
| **weights load total** | 0.0 ms | **4.4 min (266 s)** |

- **weights load total:** 4.4 min (266 s) of 5.3 min (317 s) wall
- **compute + overhead (residual):** 50.88 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved flux_1_dev_tp4sdpa_out.png |
...
- per-step = 682.3 ms/DiT-step (median 682.0, p90 682.3, n=27) — measured the SAME way as H100: inter-step deltas of a real 28-step generate (wrapping NeuronFluxBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 28 DiT calls timed; warm generate 19s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 317 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 50 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 317 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4, --attention-impl sdpa (PyTorch SDPA via XLA instead of attention_cte); tp=4 (registry default tp=8 -> 4 on trn2.3xlarge), bf16, attention_cte |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model flux_1_dev --config tp4sdpa    # true cold + warm e2e
...
#   python -m benchmark.bench --backend cuda --model flux_1_dev --config tp4sdpa   # diffusers CUDA reference adapter
```
