...
**Timestamp:** 2026-09-18 10:06 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.5; alpha sweep); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF)
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 6.43 s |
| **e2e generate — cold start** (page cache dropped) | **12.8 min (769 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 5.2 min (310 s) |
| **e2e generate — warm cache** | **56.64 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 11.67 s |

> Cold vs warm: **12.8 min (769 s) → 56.64 s** (13.6× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 460.2 ms | 459.9 ms | 460.4 ms | 459.5 ms | 16 |
| end-to-end (warm) | 56.64 s | 56.64 s | 56.64 s | 56.64 s | 1 |
...
| note | saved ltx_2_tp4tcod05_out.mp4 |
...
- per-step = 460.2 ms/DiT-step (median 459.9, p90 460.4, n=16) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronLTX2Application.forward_dit, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 17 DiT calls timed; warm generate 33s; output finite=True.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.5; alpha sweep); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF) |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model ltx_2 --config tp4tcod05    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model ltx_2 --config tp4tcod05    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model ltx_2 --config tp4tcod05   # diffusers CUDA reference adapter
```
