...
**Timestamp:** 2026-09-18 09:26 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.4; alpha sweep); tp=4, bf16, attention_cte
...
| **e2e generate — warm cache** | **110.10 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 57.57 s |

> Cold vs warm: **9.9 min (593 s) → 110.10 s** (5.4× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 815.8 ms | 815.3 ms | 817.9 ms | 814.9 ms | 14 |
| end-to-end (warm) | 110.10 s | 110.10 s | 110.10 s | 110.10 s | 1 |

**Throughput:** 1.226 DiT steps/s
...
- **compute + overhead (residual):** 71.74 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved hunyuanvideo_tp4tcod04_out.mp4 |
...
- per-step = 815.8 ms/DiT-step (median 815.3, p90 817.9, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronHunyuanVideoBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 84s; output finite=True.
...
- e2e_warm = 110 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 593 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.4; alpha sweep); tp=4, bf16, attention_cte |
...
difflet generate --model-id hunyuanvideo-community/HunyuanVideo --revision e8c2aaa66fe3742a32c11a6766aecbf07c56e773 \
    --tp-degree 4 --cp-degree 1 --height 320 --width 512 --num-frames 61 \
    --steps 20 --guidance-scale 6.0 --seed 42 --teacache-online-delta 0.4 \
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model hunyuan_video --config tp4tcod04    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model hunyuan_video --config tp4tcod04    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model hunyuan_video --config tp4tcod04   # diffusers CUDA reference adapter
```
