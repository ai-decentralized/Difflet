...
**Timestamp:** 2026-09-12 06:02 UTC
...
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 849.0 ms | 848.8 ms | 849.0 ms | 848.6 ms | 19 |
| end-to-end (warm) | 116.21 s | 116.21 s | 116.21 s | 116.21 s | 1 |

**Throughput:** 1.178 DiT steps/s
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder_clip | 377.0 ms | 602.0 ms | — | 35.01 s | 430.0 ms | **36.42 s** |
| text_encoder | 103.0 ms | 4.72 s | 114.06 s | 20.68 s | 112.77 s | **4.2 min (252 s)** |
| transformer | 16.66 s | 56.25 s | 2.3 min (141 s) | 4.0 ms | 3.7 min (225 s) | **7.3 min (438 s)** |
| vae_decoder | 845.0 ms | 348.0 ms | 60.5 min (3629 s) | 1.0 ms | 97.86 s | **62.1 min (3728 s)** |
| **Σ component builds** | | | | | | **74.3 min (4455 s)** |

> The headline **compile = 88.8 min (5327 s)** is the full `difflet compile` wall; the **Σ component builds = 74.3 min (4455 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
| note | saved hunyuanvideo_tp2cp2_out.mp4 |
...
- per-step = 849.0 ms/DiT-step (median 848.8, p90 849.0, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronHunyuanVideoBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 90s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 748 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 116 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 748 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
