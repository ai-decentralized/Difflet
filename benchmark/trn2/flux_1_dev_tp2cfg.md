...
> Best-performing configuration: tp=2 x CFG-parallel (uncond/cond on separate core pairs); tp=4 (registry default tp=8 -> 4 on trn2.3xlarge), bf16, attention_cte
...
| best-perf knobs | tp=2 x CFG-parallel (uncond/cond on separate core pairs); tp=4 (registry default tp=8 -> 4 on trn2.3xlarge), bf16, attention_cte |
...
difflet generate --model-id black-forest-labs/FLUX.1-dev --revision 3de623fc3c33e44ffbe2bad470d0f45bccf2eb21 \
    --tp-degree 2 --cp-degree 1 --cfg-parallel --height 1024 --width 1024 \
    --steps 28 --guidance-scale 2.0 --seed 42 \
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model flux_1_dev --config tp2cfg    # true cold + warm e2e
...
#   python -m benchmark.bench --backend cuda --model flux_1_dev --config tp2cfg   # diffusers CUDA reference adapter
```
