...
> Best-performing configuration: tp=2 x cp=2 (ulysses); tp=4 (registry default tp=8 -> 4 on trn2.3xlarge), bf16, attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 19.7 min (1180 s) |
| **e2e generate — cold start** (page cache dropped) | **8.2 min (491 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 7.5 min (452 s) |
| **e2e generate — warm cache** | **43.71 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 25.26 s |

> Cold vs warm: **8.2 min (491 s) → 43.71 s** (11.2× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 263.1 ms | 262.7 ms | 263.2 ms | 262.5 ms | 27 |
| end-to-end (warm) | 43.71 s | 43.71 s | 43.71 s | 43.71 s | 1 |

**Throughput:** 3.801 DiT steps/s
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder_clip | 325.0 ms | 523.0 ms | — | 48.0 ms | 105.0 ms | **1.00 s** |
| text_encoder_t5 | 361.0 ms | 701.0 ms | — | 87.0 ms | 81.0 ms | **1.23 s** |
| transformer | 1.80 s | 12.28 s | 57.99 s | 3.0 ms | 2.5 min (153 s) | **3.8 min (225 s)** |
| vae_decoder | 202.0 ms | 466.0 ms | — | 11.6 min (698 s) | 533.0 ms | **11.6 min (699 s)** |
| **Σ component builds** | | | | | | **15.4 min (926 s)** |

> The headline **compile = 19.7 min (1180 s)** is the full `difflet compile` wall; the **Σ component builds = 15.4 min (926 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
|---|---:|---:|
| text_encoder_t5 | — | 72.30 s |
| transformer (denoise loop) | — | 6.2 min (372 s) |
| text_encoder_clip | — | 970.0 ms |
| vae_decoder | — | 7.03 s |
| **weights load total** | 0.0 ms | **7.5 min (452 s)** |

- **weights load total:** 7.5 min (452 s) of 8.2 min (491 s) wall
- **compute + overhead (residual):** 39.17 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved flux_1_dev_tp2cp2_out.png |
...
- per-step = 263.1 ms/DiT-step (median 262.7, p90 263.2, n=27) — measured the SAME way as H100: inter-step deltas of a real 28-step generate (wrapping NeuronFluxBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 28 DiT calls timed; warm generate 8s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 491 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 44 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 491 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=2 x cp=2 (ulysses); tp=4 (registry default tp=8 -> 4 on trn2.3xlarge), bf16, attention_cte |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model flux_1_dev --config tp2cp2    # true cold + warm e2e
...
#   python -m benchmark.bench --backend cuda --model flux_1_dev --config tp2cp2   # diffusers CUDA reference adapter
```
