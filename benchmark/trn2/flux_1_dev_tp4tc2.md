...
**Timestamp:** 2026-09-13 03:16 UTC

> Best-performing configuration: tp=4 + TeaCache fixed cadence 2 (--teacache-cadence 2); tp=4 (registry default tp=8 -> 4 on trn2.3xlarge), bf16, attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 21.11 s |
| **e2e generate — cold start** (page cache dropped) | **5.1 min (306 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 4.5 min (270 s) |
| **e2e generate — warm cache** | **38.70 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 22.91 s |

> Cold vs warm: **5.1 min (306 s) → 38.70 s** (7.9× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 270.8 ms | 270.4 ms | 270.9 ms | 269.6 ms | 18 |
| end-to-end (warm) | 38.70 s | 38.70 s | 38.70 s | 38.70 s | 1 |
...
|---|---:|---:|
| text_encoder_t5 | — | 73.55 s |
| transformer (denoise loop) | — | 3.1 min (189 s) |
| text_encoder_clip | — | 970.0 ms |
| vae_decoder | — | 6.98 s |
| **weights load total** | 0.0 ms | **4.5 min (270 s)** |

- **weights load total:** 4.5 min (270 s) of 5.1 min (306 s) wall
- **compute + overhead (residual):** 36.24 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved flux_1_dev_tp4tc2_out.png |
...
- per-step = 270.8 ms/DiT-step (median 270.4, p90 270.9, n=18) — measured the SAME way as H100: inter-step deltas of a real 28-step generate (wrapping NeuronFluxBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 19 DiT calls timed; warm generate 5s; output finite=True.
...
- e2e_warm = 39 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 306 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache fixed cadence 2 (--teacache-cadence 2); tp=4 (registry default tp=8 -> 4 on trn2.3xlarge), bf16, attention_cte |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model flux_1_dev --config tp4tc2    # true cold + warm e2e
...
#   python -m benchmark.bench --backend cuda --model flux_1_dev --config tp4tc2   # diffusers CUDA reference adapter
```
