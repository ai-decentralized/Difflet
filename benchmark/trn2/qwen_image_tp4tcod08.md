...
**Timestamp:** 2026-09-18 11:46 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.8; alpha sweep); tp=4, bf16, joint attention via attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 17.21 s |
| **e2e generate — cold start** (page cache dropped) | **8.4 min (503 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 7.6 min (457 s) |
| **e2e generate — warm cache** | **64.06 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 36.21 s |

> Cold vs warm: **8.4 min (503 s) → 64.06 s** (7.8× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 417.9 ms | 417.7 ms | 418.1 ms | 417.1 ms | 14 |
| end-to-end (warm) | 64.06 s | 64.06 s | 64.06 s | 64.06 s | 1 |

**Throughput:** 2.393 DiT steps/s

## Compile breakdown

| component | build time |
|---|---|
| wall_total_s | 17.21 s |
...
|---|---:|---:|
| text_encoder | — | 2.0 min (123 s) |
| transformer (denoise loop) | — | 5.4 min (324 s) |
| vae_decoder | — | 10.32 s |
| **weights load total** | 0.0 ms | **7.6 min (457 s)** |

- **weights load total:** 7.6 min (457 s) of 8.4 min (503 s) wall
- **compute + overhead (residual):** 45.41 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved qwen_image_tp4tcod08_out.png |
...
- per-step = 417.9 ms/DiT-step (median 417.7, p90 418.1, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronQwenImageTransformerApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 26s; output finite=True.
...
- e2e_warm = 64 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 503 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.8; alpha sweep); tp=4, bf16, joint attention via attention_cte |
...
difflet generate --model-id Qwen/Qwen-Image --revision 75e0b4be04f60ec59a75f475837eced720f823b6 \
    --tp-degree 4 --cp-degree 1 --height 1024 --width 1024 \
    --steps 20 --guidance-scale 4.0 --seed 42 --teacache-online-delta 0.8 \
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model qwen_image --config tp4tcod08    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model qwen_image --config tp4tcod08    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model qwen_image --config tp4tcod08   # diffusers CUDA reference adapter
```
