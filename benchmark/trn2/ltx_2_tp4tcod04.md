...
**Timestamp:** 2026-09-18 09:11 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.4; alpha sweep); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF)
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 6.18 s |
| **e2e generate — cold start** (page cache dropped) | **12.8 min (769 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 5.2 min (310 s) |
| **e2e generate — warm cache** | **57.45 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 12.09 s |

> Cold vs warm: **12.8 min (769 s) → 57.45 s** (13.4× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 459.7 ms | 459.2 ms | 460.2 ms | 458.9 ms | 18 |
| end-to-end (warm) | 57.45 s | 57.45 s | 57.45 s | 57.45 s | 1 |

**Throughput:** 2.176 DiT steps/s
...
| note | saved ltx_2_tp4tcod04_out.mp4 |
...
- per-step = 459.7 ms/DiT-step (median 459.2, p90 460.2, n=18) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronLTX2Application.forward_dit, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 19 DiT calls timed; warm generate 34s; output finite=True.
...
- e2e_warm = 57 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 769 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.4; alpha sweep); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF) |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model ltx_2 --config tp4tcod04    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model ltx_2 --config tp4tcod04    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model ltx_2 --config tp4tcod04   # diffusers CUDA reference adapter
```
