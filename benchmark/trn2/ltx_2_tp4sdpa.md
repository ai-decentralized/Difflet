...
**Timestamp:** 2026-09-12 23:29 UTC

> Best-performing configuration: tp=4, --attention-impl sdpa (PyTorch SDPA via XLA instead of attention_cte); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF)
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 8.5 min (508 s) |
| **e2e generate — cold start** (page cache dropped) | **12.8 min (770 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 5.1 min (308 s) |
| **e2e generate — warm cache** | **57.59 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 10.43 s |

> Cold vs warm: **12.8 min (770 s) → 57.59 s** (13.4× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 506.8 ms | 506.6 ms | 507.1 ms | 505.1 ms | 19 |
| end-to-end (warm) | 57.59 s | 57.59 s | 57.59 s | 57.59 s | 1 |

**Throughput:** 1.973 DiT steps/s
...
|---|---:|---:|---:|---:|---:|---:|
| transformer | 83.68 s | 16.52 s | 99.01 s | 13.0 ms | 5.0 min (301 s) | **8.3 min (500 s)** |
| **Σ component builds** | | | | | | **8.3 min (500 s)** |

> The headline **compile = 8.5 min (508 s)** is the full `difflet compile` wall; the **Σ component builds = 8.3 min (500 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
|---|---:|---:|
| transformer (denoise loop) [Neuron] | — | 5.1 min (308 s) |
| **weights load total** | 0.0 ms | **5.1 min (308 s)** |

- **weights load total:** 5.1 min (308 s) of 12.8 min (770 s) wall
- **compute + overhead (residual):** 7.7 min (461 s) = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved ltx_2_tp4sdpa_out.mp4 |
...
- per-step = 506.8 ms/DiT-step (median 506.6, p90 507.1, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronLTX2Application.forward_dit, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 35s; output finite=True.
...
- e2e_cold = 770 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 58 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 770 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4, --attention-impl sdpa (PyTorch SDPA via XLA instead of attention_cte); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF) |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model ltx_2 --config tp4sdpa    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model ltx_2 --config tp4sdpa    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model ltx_2 --config tp4sdpa   # diffusers CUDA reference adapter
```
