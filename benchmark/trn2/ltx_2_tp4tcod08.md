...
**Timestamp:** 2026-09-18 11:57 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.8; alpha sweep); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF)
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 6.23 s |
| **e2e generate — cold start** (page cache dropped) | **12.8 min (768 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 5.2 min (310 s) |
| **e2e generate — warm cache** | **55.73 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 12.33 s |

> Cold vs warm: **12.8 min (768 s) → 55.73 s** (13.8× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 460.0 ms | 459.3 ms | 461.3 ms | 458.5 ms | 14 |
| end-to-end (warm) | 55.73 s | 55.73 s | 55.73 s | 55.73 s | 1 |

**Throughput:** 2.174 DiT steps/s
...
- **weights load total:** 5.2 min (310 s) of 12.8 min (768 s) wall
- **compute + overhead (residual):** 7.6 min (458 s) = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved ltx_2_tp4tcod08_out.mp4 |
...
- per-step = 460.0 ms/DiT-step (median 459.3, p90 461.3, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronLTX2Application.forward_dit, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 32s; output finite=True.
...
- e2e_cold = 768 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 56 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 768 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.8; alpha sweep); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF) |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model ltx_2 --config tp4tcod08    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model ltx_2 --config tp4tcod08    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model ltx_2 --config tp4tcod08   # diffusers CUDA reference adapter
```
