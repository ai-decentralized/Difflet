...
**Timestamp:** 2026-09-18 07:09 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.2; alpha sweep); tp=4, bf16, joint attention via attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 17.31 s |
| **e2e generate — cold start** (page cache dropped) | **8.3 min (497 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 7.5 min (450 s) |
| **e2e generate — warm cache** | **64.36 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 35.08 s |

> Cold vs warm: **8.3 min (497 s) → 64.36 s** (7.7× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 418.4 ms | 418.2 ms | 418.6 ms | 417.6 ms | 16 |
| end-to-end (warm) | 64.36 s | 64.36 s | 64.36 s | 64.36 s | 1 |

**Throughput:** 2.390 DiT steps/s

## Compile breakdown

| component | build time |
|---|---|
| wall_total_s | 17.31 s |
...
|---|---:|---:|
| text_encoder | — | 2.1 min (125 s) |
| transformer (denoise loop) | — | 5.3 min (317 s) |
| vae_decoder | — | 8.71 s |
| **weights load total** | 0.0 ms | **7.5 min (450 s)** |

- **weights load total:** 7.5 min (450 s) of 8.3 min (497 s) wall
- **compute + overhead (residual):** 47.42 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved qwen_image_tp4tcod02_out.png |
...
- per-step = 418.4 ms/DiT-step (median 418.2, p90 418.6, n=16) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronQwenImageTransformerApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 17 DiT calls timed; warm generate 28s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 497 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 64 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 497 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.2; alpha sweep); tp=4, bf16, joint attention via attention_cte |
...
difflet generate --model-id Qwen/Qwen-Image --revision 75e0b4be04f60ec59a75f475837eced720f823b6 \
    --tp-degree 4 --cp-degree 1 --height 1024 --width 1024 \
    --steps 20 --guidance-scale 4.0 --seed 42 --teacache-online-delta 0.2 \
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model qwen_image --config tp4tcod02    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model qwen_image --config tp4tcod02    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model qwen_image --config tp4tcod02   # diffusers CUDA reference adapter
```
