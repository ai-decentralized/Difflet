...
**Timestamp:** 2026-09-13 04:01 UTC

> Best-performing configuration: tp=4 + TeaCache fixed cadence 2 (--teacache-cadence 2); tp=4, bf16, attention_cte
...
| **e2e generate — warm cache** | **109.03 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 56.25 s |

> Cold vs warm: **9.9 min (594 s) → 109.03 s** (5.4× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 814.7 ms | 814.5 ms | 814.9 ms | 814.2 ms | 14 |
| end-to-end (warm) | 109.03 s | 109.03 s | 109.03 s | 109.03 s | 1 |
...
|---|---:|---:|
| text_encoder_clip | — | 3.70 s |
| text_encoder | — | 2.2 min (130 s) |
| stage_2 | — | 6.3 min (376 s) |
| vae_decoder | — | 12.10 s |
| **weights load total** | 0.0 ms | **8.7 min (521 s)** |

- **weights load total:** 8.7 min (521 s) of 9.9 min (594 s) wall
- **compute + overhead (residual):** 72.21 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved hunyuanvideo_tp4tc2_out.mp4 |
...
- per-step = 814.7 ms/DiT-step (median 814.5, p90 814.9, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronHunyuanVideoBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 82s; output finite=True.
...
| best-perf knobs | tp=4 + TeaCache fixed cadence 2 (--teacache-cadence 2); tp=4, bf16, attention_cte |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model hunyuan_video --config tp4tc2    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model hunyuan_video --config tp4tc2    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model hunyuan_video --config tp4tc2   # diffusers CUDA reference adapter
```
