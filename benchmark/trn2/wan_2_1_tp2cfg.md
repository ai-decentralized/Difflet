...
**Timestamp:** 2026-09-12 19:42 UTC

> Best-performing configuration: tp=2 x CFG-parallel (uncond/cond on separate core pairs); tp=4, bf16, single-transformer (no MoE), attention_cte, 2-stage (transformer + VAE) subprocess pipeline
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 22.6 min (1353 s) |
| **e2e generate — cold start** (page cache dropped) | **10.7 min (640 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 9.6 min (577 s) |
| **e2e generate — warm cache** | **102.50 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 59.12 s |

> Cold vs warm: **10.7 min (640 s) → 102.50 s** (6.2× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.
...
| end-to-end (warm) | 102.50 s | 102.50 s | 102.50 s | 102.50 s | 1 |

**Throughput:** 0.934 DiT steps/s
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder_t5 | 1.01 s | 922.0 ms | 10.97 s | 1.0 ms | 58.27 s | **71.18 s** |
| transformer | 3.06 s | 42.84 s | 117.44 s | 3.0 ms | 4.1 min (246 s) | **6.8 min (409 s)** |
| **Σ component builds** | | | | | | **8.0 min (480 s)** |

> The headline **compile = 22.6 min (1353 s)** is the full `difflet compile` wall; the **Σ component builds = 8.0 min (480 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
|---|---:|---:|
| text_encoder (UMT5) | — | 90.34 s |
| transformer (denoise loop) | — | 7.6 min (455 s) |
| vae_decoder | — | 31.06 s |
| **weights load total** | 0.0 ms | **9.6 min (577 s)** |

- **weights load total:** 9.6 min (577 s) of 10.7 min (640 s) wall
- **compute + overhead (residual):** 62.92 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved wan2_1_t2v_14b_diffusers_tp2cfg_out.mp4 |
...
- per-step = 1070.6 ms/DiT-step (median 1070.4, p90 1070.8, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronWanBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 57s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 640 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 102 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 640 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=2 x CFG-parallel (uncond/cond on separate core pairs); tp=4, bf16, single-transformer (no MoE), attention_cte, 2-stage (transformer + VAE) subprocess pipeline |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model wan_2_1 --config tp2cfg    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model wan_2_1 --config tp2cfg    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model wan_2_1 --config tp2cfg   # diffusers CUDA reference adapter
```
