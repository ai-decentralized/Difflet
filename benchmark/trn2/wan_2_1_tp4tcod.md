...
**Timestamp:** 2026-09-13 05:10 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.6); tp=4, bf16, single-transformer (no MoE), attention_cte, 2-stage (transformer + VAE) subprocess pipeline
...
| **e2e generate — cold start** (page cache dropped) | **6.7 min (404 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 5.9 min (354 s) |
| **e2e generate — warm cache** | **79.87 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 48.95 s |

> Cold vs warm: **6.7 min (404 s) → 79.87 s** (5.1× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 576.1 ms | 575.9 ms | 577.0 ms | 575.3 ms | 14 |
| end-to-end (warm) | 79.87 s | 79.87 s | 79.87 s | 79.87 s | 1 |
...
|---|---:|---:|
| text_encoder (UMT5) | — | 88.04 s |
| transformer (denoise loop) | — | 3.9 min (236 s) |
| vae_decoder | — | 30.30 s |
| **weights load total** | 0.0 ms | **5.9 min (354 s)** |

- **weights load total:** 5.9 min (354 s) of 6.7 min (404 s) wall
- **compute + overhead (residual):** 50.12 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved wan2_1_t2v_14b_diffusers_tp4tcod_out.mp4 |
...
- per-step = 576.1 ms/DiT-step (median 575.9, p90 577.0, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronWanBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 32s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 404 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 80 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 404 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.6); tp=4, bf16, single-transformer (no MoE), attention_cte, 2-stage (transformer + VAE) subprocess pipeline |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model wan_2_1 --config tp4tcod    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model wan_2_1 --config tp4tcod    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model wan_2_1 --config tp4tcod   # diffusers CUDA reference adapter
```
