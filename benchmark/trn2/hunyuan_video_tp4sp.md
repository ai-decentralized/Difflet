...
**Timestamp:** 2026-09-12 16:31 UTC
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 82.7 min (4961 s) |
| **e2e generate — cold start** (page cache dropped) | **9.9 min (596 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 8.7 min (520 s) |
| **e2e generate — warm cache** | **111.90 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 55.69 s |

> Cold vs warm: **9.9 min (596 s) → 111.90 s** (5.3× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 790.6 ms | 790.3 ms | 790.5 ms | 790.0 ms | 19 |
| end-to-end (warm) | 111.90 s | 111.90 s | 111.90 s | 111.90 s | 1 |

**Throughput:** 1.265 DiT steps/s
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder_clip | 274.0 ms | 473.0 ms | — | 19.50 s | 91.0 ms | **20.34 s** |
| text_encoder | 96.0 ms | 3.08 s | 77.27 s | 8.43 s | 63.78 s | **2.5 min (153 s)** |
| transformer | 15.88 s | 55.56 s | 3.1 min (184 s) | 4.0 ms | 2.2 min (133 s) | **6.5 min (388 s)** |
| vae_decoder | 1.74 s | 437.0 ms | 60.8 min (3647 s) | 1.0 ms | 97.89 s | **62.4 min (3747 s)** |
| **Σ component builds** | | | | | | **71.8 min (4308 s)** |

> The headline **compile = 82.7 min (4961 s)** is the full `difflet compile` wall; the **Σ component builds = 71.8 min (4308 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
|---|---:|---:|
| text_encoder_clip | — | 3.51 s |
| text_encoder | — | 2.2 min (130 s) |
| stage_2 | — | 6.2 min (374 s) |
| vae_decoder | — | 12.44 s |
| **weights load total** | 0.0 ms | **8.7 min (520 s)** |

- **weights load total:** 8.7 min (520 s) of 9.9 min (596 s) wall
- **compute + overhead (residual):** 75.67 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved hunyuanvideo_tp4sp_out.mp4 |
...
- per-step = 790.6 ms/DiT-step (median 790.3, p90 790.5, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronHunyuanVideoBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 85s; output finite=True.
...
- e2e_warm = 112 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 596 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
