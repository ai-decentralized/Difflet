...
**Timestamp:** 2026-09-12 18:07 UTC

> Best-performing configuration: tp=4 + sequence parallel; tp=4, bf16, single-transformer (no MoE), attention_cte, 2-stage (transformer + VAE) subprocess pipeline
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 16.1 min (967 s) |
| **e2e generate — cold start** (page cache dropped) | **6.8 min (406 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 5.9 min (353 s) |
| **e2e generate — warm cache** | **79.38 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 47.32 s |

> Cold vs warm: **6.8 min (406 s) → 79.38 s** (5.1× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 578.2 ms | 578.1 ms | 578.4 ms | 577.7 ms | 19 |
| end-to-end (warm) | 79.38 s | 79.38 s | 79.38 s | 79.38 s | 1 |
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder_t5 | 688.0 ms | 962.0 ms | 6.60 s | 1.0 ms | 29.50 s | **37.75 s** |
| transformer | 2.08 s | 23.02 s | 66.91 s | 3.0 ms | 2.7 min (162 s) | **4.2 min (254 s)** |
| **Σ component builds** | | | | | | **4.9 min (292 s)** |

> The headline **compile = 16.1 min (967 s)** is the full `difflet compile` wall; the **Σ component builds = 4.9 min (292 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
|---|---:|---:|
| text_encoder (UMT5) | — | 89.31 s |
| transformer (denoise loop) | — | 3.9 min (235 s) |
| vae_decoder | — | 29.04 s |
| **weights load total** | 0.0 ms | **5.9 min (353 s)** |

- **weights load total:** 5.9 min (353 s) of 6.8 min (406 s) wall
- **compute + overhead (residual):** 52.63 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved wan2_1_t2v_14b_diffusers_tp4sp_out.mp4 |
...
- per-step = 578.2 ms/DiT-step (median 578.1, p90 578.4, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronWanBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 36s; output finite=True.
...
| best-perf knobs | tp=4 + sequence parallel; tp=4, bf16, single-transformer (no MoE), attention_cte, 2-stage (transformer + VAE) subprocess pipeline |
...
difflet compile  --model-id Wan-AI/Wan2.1-T2V-14B-Diffusers --revision 38ec498cb3208fb688890f8cc7e94ede2cbd7f68 \
    --tp-degree 4 --cp-degree 1 --sp --height 480 --width 832 --num-frames 9
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model wan_2_1 --config tp4sp    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model wan_2_1 --config tp4sp    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model wan_2_1 --config tp4sp   # diffusers CUDA reference adapter
```
