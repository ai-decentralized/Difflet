...
**Timestamp:** 2026-09-13 03:46 UTC

> Best-performing configuration: tp=4 + TeaCache fixed cadence 2 (--teacache-cadence 2); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF)
...
| **e2e generate — warm cache** | **55.77 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 11.94 s |

> Cold vs warm: **12.8 min (768 s) → 55.77 s** (13.8× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 459.4 ms | 459.0 ms | 460.8 ms | 458.3 ms | 14 |
| end-to-end (warm) | 55.77 s | 55.77 s | 55.77 s | 55.77 s | 1 |
...
| note | saved ltx_2_tp4tc2_out.mp4 |
...
- per-step = 459.4 ms/DiT-step (median 459.0, p90 460.8, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronLTX2Application.forward_dit, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 31s; output finite=True.
...
| best-perf knobs | tp=4 + TeaCache fixed cadence 2 (--teacache-cadence 2); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF) |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model ltx_2 --config tp4tc2    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model ltx_2 --config tp4tc2    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model ltx_2 --config tp4tc2   # diffusers CUDA reference adapter
```
