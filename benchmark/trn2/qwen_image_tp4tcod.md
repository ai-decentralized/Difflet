...
**Timestamp:** 2026-09-13 04:30 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.6); tp=4, bf16, joint attention via attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 19.01 s |
| **e2e generate — cold start** (page cache dropped) | **8.2 min (494 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 7.4 min (447 s) |
| **e2e generate — warm cache** | **64.00 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 35.78 s |

> Cold vs warm: **8.2 min (494 s) → 64.00 s** (7.7× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 417.5 ms | 417.4 ms | 417.6 ms | 416.9 ms | 14 |
| end-to-end (warm) | 64.00 s | 64.00 s | 64.00 s | 64.00 s | 1 |

**Throughput:** 2.395 DiT steps/s

## Compile breakdown

| component | build time |
|---|---|
| wall_total_s | 19.01 s |
...
- **weights load total:** 7.4 min (447 s) of 8.2 min (494 s) wall
- **compute + overhead (residual):** 47.55 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved qwen_image_tp4tcod_out.png |
...
- per-step = 417.5 ms/DiT-step (median 417.4, p90 417.6, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronQwenImageTransformerApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 26s; output finite=True.
...
- e2e_warm = 64 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 494 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.6); tp=4, bf16, joint attention via attention_cte |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model qwen_image --config tp4tcod    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model qwen_image --config tp4tcod    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model qwen_image --config tp4tcod   # diffusers CUDA reference adapter
```
