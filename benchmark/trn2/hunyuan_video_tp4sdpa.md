...
**Timestamp:** 2026-09-12 23:52 UTC

> Best-performing configuration: tp=4, --attention-impl sdpa (PyTorch SDPA via XLA instead of attention_cte); tp=4, bf16, attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 85.7 min (5142 s) |
| **e2e generate — cold start** (page cache dropped) | **10.8 min (651 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 8.6 min (516 s) |
| **e2e generate — warm cache** | **2.8 min (168 s)** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 53.28 s |

> Cold vs warm: **10.8 min (651 s) → 2.8 min (168 s)** (3.9× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 3.64 s | 3.64 s | 3.64 s | 3.64 s | 19 |
| end-to-end (warm) | 2.8 min (168 s) | 2.8 min (168 s) | 2.8 min (168 s) | 2.8 min (168 s) | 1 |

**Throughput:** 0.275 DiT steps/s
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder_clip | 273.0 ms | 499.0 ms | — | 20.25 s | 95.0 ms | **21.11 s** |
| text_encoder | 100.0 ms | 3.02 s | 77.69 s | 8.38 s | 63.97 s | **2.6 min (153 s)** |
| transformer | 16.02 s | 4.25 s | 12.6 min (754 s) | 5.0 ms | 3.2 min (192 s) | **16.1 min (967 s)** |
| vae_decoder | 1.04 s | 276.0 ms | 60.5 min (3632 s) | 1.0 ms | 99.23 s | **62.2 min (3732 s)** |
| **Σ component builds** | | | | | | **81.2 min (4873 s)** |

> The headline **compile = 85.7 min (5142 s)** is the full `difflet compile` wall; the **Σ component builds = 81.2 min (4873 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
|---|---:|---:|
| text_encoder_clip | — | 4.09 s |
| text_encoder | — | 2.1 min (129 s) |
| stage_2 | — | 6.2 min (371 s) |
| vae_decoder | — | 12.62 s |
| **weights load total** | 0.0 ms | **8.6 min (516 s)** |

- **weights load total:** 8.6 min (516 s) of 10.8 min (651 s) wall
- **compute + overhead (residual):** 2.2 min (134 s) = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved hunyuanvideo_tp4sdpa_out.mp4 |
...
- per-step = 3641.3 ms/DiT-step (median 3641.1, p90 3641.7, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronHunyuanVideoBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 134s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 651 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 168 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 651 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4, --attention-impl sdpa (PyTorch SDPA via XLA instead of attention_cte); tp=4, bf16, attention_cte |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model hunyuan_video --config tp4sdpa    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model hunyuan_video --config tp4sdpa    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model hunyuan_video --config tp4sdpa   # diffusers CUDA reference adapter
```
