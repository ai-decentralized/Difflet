...
**Timestamp:** 2026-09-13 04:56 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.6); tp=4, bf16, attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 17.30 s |
| **e2e generate — cold start** (page cache dropped) | **9.9 min (596 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 8.7 min (523 s) |
| **e2e generate — warm cache** | **109.14 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 56.09 s |

> Cold vs warm: **9.9 min (596 s) → 109.14 s** (5.5× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 814.9 ms | 814.6 ms | 815.1 ms | 814.2 ms | 14 |
| end-to-end (warm) | 109.14 s | 109.14 s | 109.14 s | 109.14 s | 1 |
...
|---|---:|---:|
| text_encoder_clip | — | 3.52 s |
| text_encoder | — | 2.2 min (130 s) |
| stage_2 | — | 6.3 min (377 s) |
| vae_decoder | — | 12.41 s |
| **weights load total** | 0.0 ms | **8.7 min (523 s)** |

- **weights load total:** 8.7 min (523 s) of 9.9 min (596 s) wall
- **compute + overhead (residual):** 73.10 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved hunyuanvideo_tp4tcod_out.mp4 |
...
- per-step = 814.9 ms/DiT-step (median 814.6, p90 815.1, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronHunyuanVideoBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 84s; output finite=True.
...
- e2e_warm = 109 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 596 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.6); tp=4, bf16, attention_cte |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model hunyuan_video --config tp4tcod    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model hunyuan_video --config tp4tcod    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model hunyuan_video --config tp4tcod   # diffusers CUDA reference adapter
```
