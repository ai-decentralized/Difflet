...
| phase | time |
|---|---|
| compile (AOT, one-time) | 82.3 min (4935 s) |
| **e2e generate — cold start** (page cache dropped) | **9.9 min (594 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 8.6 min (519 s) |
| **e2e generate — warm cache** | **114.65 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 57.73 s |

> Cold vs warm: **9.9 min (594 s) → 114.65 s** (5.2× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 814.1 ms | 813.9 ms | 814.1 ms | 813.7 ms | 19 |
| end-to-end (warm) | 114.65 s | 114.65 s | 114.65 s | 114.65 s | 1 |

**Throughput:** 1.228 DiT steps/s
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder_clip | 275.0 ms | 516.0 ms | — | 20.31 s | 93.0 ms | **21.19 s** |
| text_encoder | 97.0 ms | 3.07 s | 76.71 s | 8.38 s | 64.10 s | **2.5 min (152 s)** |
| transformer | 16.08 s | 56.56 s | 3.2 min (190 s) | 4.0 ms | 3.3 min (195 s) | **7.6 min (457 s)** |
| vae_decoder | 1.80 s | 586.0 ms | 60.5 min (3630 s) | 1.0 ms | 97.87 s | **62.2 min (3731 s)** |
| **Σ component builds** | | | | | | **72.7 min (4362 s)** |

> The headline **compile = 82.3 min (4935 s)** is the full `difflet compile` wall; the **Σ component builds = 72.7 min (4362 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
|---|---:|---:|
| text_encoder_clip | — | 3.47 s |
| text_encoder | — | 2.2 min (129 s) |
| stage_2 | — | 6.2 min (374 s) |
| vae_decoder | — | 11.81 s |
| **weights load total** | 0.0 ms | **8.6 min (519 s)** |

- **weights load total:** 8.6 min (519 s) of 9.9 min (594 s) wall
- **compute + overhead (residual):** 75.06 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
- per-step = 814.1 ms/DiT-step (median 813.9, p90 814.1, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronHunyuanVideoBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 87s; output finite=True.
...
- e2e_warm = 115 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 594 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
