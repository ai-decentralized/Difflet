...
| **e2e generate — warm cache** | **2.4 min (144 s)** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 42.94 s |

> Cold vs warm: **11.1 min (667 s) → 2.4 min (144 s)** (4.6× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.
...
| end-to-end (warm) | 2.4 min (144 s) | 2.4 min (144 s) | 2.4 min (145 s) | 2.4 min (143 s) | 3 |
...
- e2e_warm = 144 s (n=3; reported after 2 discarded cache-warming run(s) so the OS page cache is warm). The difflet CLI reloads weights every process, so 'warm' = warm disk cache -> faster load, not a resident model; cf. e2e cold and the load/compute breakdown.
