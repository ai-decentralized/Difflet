...
**Timestamp:** 2026-09-12 15:54 UTC
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 26.4 min (1586 s) |
| **e2e generate — cold start** (page cache dropped) | **8.5 min (508 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 7.7 min (461 s) |
| **e2e generate — warm cache** | **64.56 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 35.36 s |

> Cold vs warm: **8.5 min (508 s) → 64.56 s** (7.9× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 365.6 ms | 365.2 ms | 366.8 ms | 364.5 ms | 19 |
| end-to-end (warm) | 64.56 s | 64.56 s | 64.56 s | 64.56 s | 1 |

**Throughput:** 2.736 DiT steps/s
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder | 172.0 ms | 3.89 s | 59.66 s | 7.25 s | 32.52 s | **103.50 s** |
| transformer | 86.74 s | 17.21 s | 45.20 s | 7.0 ms | 2.6 min (154 s) | **5.1 min (304 s)** |
| vae_decoder | 338.0 ms | 437.0 ms | 6.7 min (401 s) | 0.0 ms | 9.93 s | **6.9 min (412 s)** |
| **Σ component builds** | | | | | | **13.6 min (819 s)** |

> The headline **compile = 26.4 min (1586 s)** is the full `difflet compile` wall; the **Σ component builds = 13.6 min (819 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
| transformer (denoise loop) | — | 5.5 min (328 s) |
| vae_decoder | — | 8.95 s |
| **weights load total** | 0.0 ms | **7.7 min (461 s)** |

- **weights load total:** 7.7 min (461 s) of 8.5 min (508 s) wall
- **compute + overhead (residual):** 46.90 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved qwen_image_tp4sp_out.png |
...
- per-step = 365.6 ms/DiT-step (median 365.2, p90 366.8, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronQwenImageTransformerApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 31s; output finite=True.
...
- e2e_warm = 65 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 508 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
