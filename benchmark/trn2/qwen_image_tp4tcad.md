...
**Timestamp:** 2026-09-17 16:08 UTC

> Best-performing configuration: tp=4 + TeaCache calibrated adaptive (--teacache-speedup at cadence 2's skip budget, --teacache-calibration per model); tp=4, bf16, joint attention via attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 16.95 s |
| **e2e generate — cold start** (page cache dropped) | **8.5 min (508 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 7.7 min (460 s) |
| **e2e generate — warm cache** | **68.22 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 39.76 s |

> Cold vs warm: **8.5 min (508 s) → 68.22 s** (7.4× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 420.5 ms | 420.0 ms | 422.1 ms | 418.9 ms | 14 |
| end-to-end (warm) | 68.22 s | 68.22 s | 68.22 s | 68.22 s | 1 |

**Throughput:** 2.378 DiT steps/s

## Compile breakdown

| component | build time |
|---|---|
| wall_total_s | 16.95 s |
...
| transformer (denoise loop) | — | 5.4 min (327 s) |
| transformer (denoise loop) | — | 3.50 s |
| vae_decoder | — | 8.95 s |
| **weights load total** | 0.0 ms | **7.7 min (460 s)** |

- **weights load total:** 7.7 min (460 s) of 8.5 min (508 s) wall
- **compute + overhead (residual):** 48.23 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved qwen_image_tp4tcad_out.png |
...
- per-step = 420.5 ms/DiT-step (median 420.0, p90 422.1, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronQwenImageTransformerApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 29s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 508 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 68 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 508 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache calibrated adaptive (--teacache-speedup at cadence 2's skip budget, --teacache-calibration per model); tp=4, bf16, joint attention via attention_cte |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model qwen_image --config tp4tcad    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model qwen_image --config tp4tcad    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model qwen_image --config tp4tcad   # diffusers CUDA reference adapter
```
