...
**Timestamp:** 2026-09-13 04:15 UTC
...
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 48.09 s |

> Cold vs warm: **6.7 min (403 s) → 78.27 s** (5.1× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 576.9 ms | 576.9 ms | 577.7 ms | 575.8 ms | 14 |
| end-to-end (warm) | 78.27 s | 78.27 s | 78.27 s | 78.27 s | 1 |

**Throughput:** 1.733 DiT steps/s

## Compile breakdown

| component | build time |
|---|---|
| wall_total_s | 12.14 s |
...
|---|---:|---:|
| text_encoder (UMT5) | — | 87.44 s |
| transformer (denoise loop) | — | 3.9 min (236 s) |
| vae_decoder | — | 29.54 s |
| **weights load total** | 0.0 ms | **5.9 min (353 s)** |

- **weights load total:** 5.9 min (353 s) of 6.7 min (403 s) wall
- **compute + overhead (residual):** 50.16 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved wan2_1_t2v_14b_diffusers_tp4tc2_out.mp4 |
...
- per-step = 576.9 ms/DiT-step (median 576.9, p90 577.7, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronWanBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 33s; output finite=True.
...
- e2e_warm = 78 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 403 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
