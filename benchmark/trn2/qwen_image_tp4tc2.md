...
> Best-performing configuration: tp=4 + TeaCache fixed cadence 2 (--teacache-cadence 2); tp=4, bf16, joint attention via attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 12.9 min (772 s) |
| **e2e generate — cold start** (page cache dropped) | **8.2 min (493 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 7.4 min (447 s) |
| **e2e generate — warm cache** | **62.09 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 34.07 s |

> Cold vs warm: **8.2 min (493 s) → 62.09 s** (7.9× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 417.4 ms | 417.1 ms | 417.5 ms | 416.7 ms | 14 |
| end-to-end (warm) | 62.09 s | 62.09 s | 62.09 s | 62.09 s | 1 |
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder | 186.0 ms | 3.81 s | 60.55 s | 7.19 s | 33.13 s | **104.86 s** |
| vae_decoder | 338.0 ms | 219.0 ms | 6.8 min (407 s) | 0.0 ms | 9.70 s | **6.9 min (417 s)** |
| **Σ component builds** | | | | | | **8.7 min (522 s)** |

> The headline **compile = 12.9 min (772 s)** is the full `difflet compile` wall; the **Σ component builds = 8.7 min (522 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
|---|---:|---:|
| text_encoder | — | 2.0 min (121 s) |
| transformer (denoise loop) | — | 5.3 min (317 s) |
| vae_decoder | — | 8.34 s |
| **weights load total** | 0.0 ms | **7.4 min (447 s)** |

- **weights load total:** 7.4 min (447 s) of 8.2 min (493 s) wall
- **compute + overhead (residual):** 45.94 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved qwen_image_tp4tc2_out.png |
...
- per-step = 417.4 ms/DiT-step (median 417.1, p90 417.5, n=14) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronQwenImageTransformerApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 15 DiT calls timed; warm generate 25s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 493 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 62 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 493 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache fixed cadence 2 (--teacache-cadence 2); tp=4, bf16, joint attention via attention_cte |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model qwen_image --config tp4tc2    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model qwen_image --config tp4tc2    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model qwen_image --config tp4tc2   # diffusers CUDA reference adapter
```
