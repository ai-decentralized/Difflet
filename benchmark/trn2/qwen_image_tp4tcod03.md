...
**Timestamp:** 2026-09-18 08:05 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.3; alpha sweep); tp=4, bf16, joint attention via attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 17.96 s |
| **e2e generate — cold start** (page cache dropped) | **8.4 min (502 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 7.6 min (455 s) |
| **e2e generate — warm cache** | **66.93 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 37.93 s |

> Cold vs warm: **8.4 min (502 s) → 66.93 s** (7.5× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 417.9 ms | 417.7 ms | 418.1 ms | 417.3 ms | 15 |
| end-to-end (warm) | 66.93 s | 66.93 s | 66.93 s | 66.93 s | 1 |
...
| vae_decoder | — | 8.41 s |
| **weights load total** | 0.0 ms | **7.6 min (455 s)** |

- **weights load total:** 7.6 min (455 s) of 8.4 min (502 s) wall
- **compute + overhead (residual):** 46.20 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved qwen_image_tp4tcod03_out.png |
...
- per-step = 417.9 ms/DiT-step (median 417.7, p90 418.1, n=15) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronQwenImageTransformerApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 16 DiT calls timed; warm generate 27s; output finite=True.
...
- e2e_warm = 67 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 502 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.3; alpha sweep); tp=4, bf16, joint attention via attention_cte |
...
difflet generate --model-id Qwen/Qwen-Image --revision 75e0b4be04f60ec59a75f475837eced720f823b6 \
    --tp-degree 4 --cp-degree 1 --height 1024 --width 1024 \
    --steps 20 --guidance-scale 4.0 --seed 42 --teacache-online-delta 0.3 \
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model qwen_image --config tp4tcod03    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model qwen_image --config tp4tcod03    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model qwen_image --config tp4tcod03   # diffusers CUDA reference adapter
```
