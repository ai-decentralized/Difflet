...
> Best-performing configuration: tp=2 x cp=2 (ulysses); tp=4, bf16, single-transformer (no MoE), attention_cte, 2-stage (transformer + VAE) subprocess pipeline
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 24.8 min (1487 s) |
| **e2e generate — cold start** (page cache dropped) | **10.4 min (623 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 9.5 min (570 s) |
| **e2e generate — warm cache** | **87.01 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 54.18 s |

> Cold vs warm: **10.4 min (623 s) → 87.01 s** (7.2× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 575.5 ms | 575.3 ms | 575.9 ms | 575.0 ms | 19 |
| end-to-end (warm) | 87.01 s | 87.01 s | 87.01 s | 87.01 s | 1 |

**Throughput:** 1.738 DiT steps/s
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder_t5 | 972.0 ms | 879.0 ms | 10.32 s | 1.0 ms | 53.72 s | **65.89 s** |
| transformer | 3.96 s | 26.83 s | 86.10 s | 3.0 ms | 4.3 min (260 s) | **6.3 min (377 s)** |
| **Σ component builds** | | | | | | **7.4 min (443 s)** |

> The headline **compile = 24.8 min (1487 s)** is the full `difflet compile` wall; the **Σ component builds = 7.4 min (443 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
|---|---:|---:|
| text_encoder (UMT5) | — | 89.82 s |
| transformer (denoise loop) | — | 7.5 min (451 s) |
| vae_decoder | — | 29.43 s |
| **weights load total** | 0.0 ms | **9.5 min (570 s)** |

- **weights load total:** 9.5 min (570 s) of 10.4 min (623 s) wall
- **compute + overhead (residual):** 52.71 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved wan2_1_t2v_14b_diffusers_tp2cp2_out.mp4 |
...
- per-step = 575.5 ms/DiT-step (median 575.3, p90 575.9, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronWanBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 39s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 623 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 87 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 623 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=2 x cp=2 (ulysses); tp=4, bf16, single-transformer (no MoE), attention_cte, 2-stage (transformer + VAE) subprocess pipeline |
...
difflet compile  --model-id Wan-AI/Wan2.1-T2V-14B-Diffusers --revision 38ec498cb3208fb688890f8cc7e94ede2cbd7f68 \
    --tp-degree 2 --cp-degree 2 --cp-mode ulysses --height 480 --width 832 --num-frames 9
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model wan_2_1 --config tp2cp2    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model wan_2_1 --config tp2cp2    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model wan_2_1 --config tp2cp2   # diffusers CUDA reference adapter
```
