#!/usr/bin/env bash
# Serving-layer benchmark on trn2 (SERVE_SUFFIX=_warm re-runs against an existing serving generation:
...
for m in "${MODELS[@]}"; do
  if [[ -f "$OUT/${m}_tp4${SUFFIX}.json" ]]; then echo "[serve] $m already measured, skipping"; continue; fi
  ...
  "$PY" -m benchmark.serve_bench --model "$m" --port "$PORT" --levels "$LEVELS" --requests "$nreq" \
    --warmup 2 --out "$OUT/${m}_tp4${SUFFIX}.json" --phase-file "$phase" --sampler-jsonl "$LOG/${m}_resources.jsonl" \
