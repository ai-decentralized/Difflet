...
> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.2; alpha sweep); tp=4, bf16, attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 17.41 s |
| **e2e generate — cold start** (page cache dropped) | **9.9 min (595 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 8.7 min (523 s) |
| **e2e generate — warm cache** | **110.55 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 56.92 s |

> Cold vs warm: **9.9 min (595 s) → 110.55 s** (5.4× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 815.5 ms | 815.3 ms | 815.8 ms | 814.7 ms | 15 |
| end-to-end (warm) | 110.55 s | 110.55 s | 110.55 s | 110.55 s | 1 |
...
|---|---:|---:|
| text_encoder_clip | — | 5.50 s |
| text_encoder | — | 2.2 min (130 s) |
| stage_2 | — | 6.2 min (375 s) |
| vae_decoder | — | 12.79 s |
| **weights load total** | 0.0 ms | **8.7 min (523 s)** |

- **weights load total:** 8.7 min (523 s) of 9.9 min (595 s) wall
- **compute + overhead (residual):** 72.50 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved hunyuanvideo_tp4tcod02_out.mp4 |
...
- per-step = 815.5 ms/DiT-step (median 815.3, p90 815.8, n=15) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronHunyuanVideoBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 16 DiT calls timed; warm generate 84s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 595 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 111 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 595 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.2; alpha sweep); tp=4, bf16, attention_cte |
...
difflet generate --model-id hunyuanvideo-community/HunyuanVideo --revision e8c2aaa66fe3742a32c11a6766aecbf07c56e773 \
    --tp-degree 4 --cp-degree 1 --height 320 --width 512 --num-frames 61 \
    --steps 20 --guidance-scale 6.0 --seed 42 --teacache-online-delta 0.2 \
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model hunyuan_video --config tp4tcod02    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model hunyuan_video --config tp4tcod02    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model hunyuan_video --config tp4tcod02   # diffusers CUDA reference adapter
```
