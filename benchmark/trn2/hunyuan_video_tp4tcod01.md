...
**Timestamp:** 2026-09-18 12:42 UTC
...
| **e2e generate — warm cache** | **112.75 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 58.66 s |

> Cold vs warm: **9.9 min (595 s) → 112.75 s** (5.3× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 815.5 ms | 815.3 ms | 816.2 ms | 814.8 ms | 16 |
| end-to-end (warm) | 112.75 s | 112.75 s | 112.75 s | 112.75 s | 1 |
...
|---|---:|---:|
| text_encoder_clip | — | 4.91 s |
| text_encoder | — | 2.1 min (128 s) |
| stage_2 | — | 6.3 min (375 s) |
| vae_decoder | — | 12.40 s |
| **weights load total** | 0.0 ms | **8.7 min (521 s)** |

- **weights load total:** 8.7 min (521 s) of 9.9 min (595 s) wall
- **compute + overhead (residual):** 73.92 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved hunyuanvideo_tp4tcod01_out.mp4 |
...
- per-step = 815.5 ms/DiT-step (median 815.3, p90 816.2, n=16) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronHunyuanVideoBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 17 DiT calls timed; warm generate 84s; output finite=True.
...
- e2e_warm = 113 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 595 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
