...
**Timestamp:** 2026-09-13 02:06 UTC

> Best-performing configuration: tp=4 at guidance 2.0 (two sequential CFG branches; baseline for tp2cfg); tp=4, bf16, single-transformer (no MoE), attention_cte, 2-stage (transformer + VAE) subprocess pipeline
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 12.04 s |
| **e2e generate — cold start** (page cache dropped) | **7.0 min (419 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 5.9 min (354 s) |
| **e2e generate — warm cache** | **93.86 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 50.10 s |

> Cold vs warm: **7.0 min (419 s) → 93.86 s** (4.5× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 575.8 ms | 574.7 ms | 575.8 ms | 573.5 ms | 39 |
| end-to-end (warm) | 93.86 s | 93.86 s | 93.86 s | 93.86 s | 1 |

**Throughput:** 1.737 DiT steps/s

## Compile breakdown

| component | build time |
|---|---|
| wall_total_s | 12.04 s |
...
|---|---:|---:|
| text_encoder (UMT5) | — | 87.72 s |
| transformer (denoise loop) | — | 3.9 min (235 s) |
| vae_decoder | — | 30.80 s |
| **weights load total** | 0.0 ms | **5.9 min (354 s)** |

- **weights load total:** 5.9 min (354 s) of 7.0 min (419 s) wall
- **compute + overhead (residual):** 65.23 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved wan2_1_t2v_14b_diffusers_tp4cfg2_out.mp4 |
...
- per-step = 575.8 ms/DiT-step (median 574.7, p90 575.8, n=39) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronWanBackboneApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 40 DiT calls timed; warm generate 45s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 419 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 94 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 419 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 at guidance 2.0 (two sequential CFG branches; baseline for tp2cfg); tp=4, bf16, single-transformer (no MoE), attention_cte, 2-stage (transformer + VAE) subprocess pipeline |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model wan_2_1 --config tp4cfg2    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model wan_2_1 --config tp4cfg2    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model wan_2_1 --config tp4cfg2   # diffusers CUDA reference adapter
```
