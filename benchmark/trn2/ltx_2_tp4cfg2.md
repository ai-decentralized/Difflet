...
**Timestamp:** 2026-09-13 01:49 UTC

> Best-performing configuration: tp=4 at guidance 2.0 (two sequential CFG branches; baseline for tp2cfg); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF)
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 6.73 s |
| **e2e generate — cold start** (page cache dropped) | **13.2 min (792 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 5.2 min (310 s) |
| **e2e generate — warm cache** | **77.09 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 11.10 s |

> Cold vs warm: **13.2 min (792 s) → 77.09 s** (10.3× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 459.1 ms | 459.4 ms | 459.9 ms | 457.4 ms | 39 |
| end-to-end (warm) | 77.09 s | 77.09 s | 77.09 s | 77.09 s | 1 |

**Throughput:** 2.178 DiT steps/s
...
- **weights load total:** 5.2 min (310 s) of 13.2 min (792 s) wall
...
| note | saved ltx_2_tp4cfg2_out.mp4 |
...
- per-step = 459.1 ms/DiT-step (median 459.4, p90 459.9, n=39) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronLTX2Application.forward_dit, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 40 DiT calls timed; warm generate 55s; output finite=True.
...
- e2e_cold = 792 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 77 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 792 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 at guidance 2.0 (two sequential CFG branches; baseline for tp2cfg); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF) |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model ltx_2 --config tp4cfg2    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model ltx_2 --config tp4cfg2    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model ltx_2 --config tp4cfg2   # diffusers CUDA reference adapter
```
