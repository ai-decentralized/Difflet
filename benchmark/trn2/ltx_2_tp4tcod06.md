...
**Timestamp:** 2026-09-18 11:02 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.6; alpha sweep); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF)
...
| **e2e generate — cold start** (page cache dropped) | **12.9 min (774 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 5.2 min (310 s) |
| **e2e generate — warm cache** | **55.98 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 11.28 s |

> Cold vs warm: **12.9 min (774 s) → 55.98 s** (13.8× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 459.8 ms | 459.1 ms | 461.6 ms | 458.6 ms | 15 |
| end-to-end (warm) | 55.98 s | 55.98 s | 55.98 s | 55.98 s | 1 |

**Throughput:** 2.175 DiT steps/s
...
- **weights load total:** 5.2 min (310 s) of 12.9 min (774 s) wall
- **compute + overhead (residual):** 7.7 min (464 s) = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved ltx_2_tp4tcod06_out.mp4 |
...
- per-step = 459.8 ms/DiT-step (median 459.1, p90 461.6, n=15) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronLTX2Application.forward_dit, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 16 DiT calls timed; warm generate 31s; output finite=True.
...
- e2e_cold = 774 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 56 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 774 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.6; alpha sweep); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF) |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model ltx_2 --config tp4tcod06    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model ltx_2 --config tp4tcod06    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model ltx_2 --config tp4tcod06   # diffusers CUDA reference adapter
```
