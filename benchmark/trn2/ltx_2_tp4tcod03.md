...
**Timestamp:** 2026-09-18 08:16 UTC

> Best-performing configuration: tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.3; alpha sweep); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF)
...
| **e2e generate — cold start** (page cache dropped) | **12.9 min (771 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 5.2 min (310 s) |
| **e2e generate — warm cache** | **58.44 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 12.32 s |

> Cold vs warm: **12.9 min (771 s) → 58.44 s** (13.2× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 459.4 ms | 458.8 ms | 460.1 ms | 458.1 ms | 19 |
| end-to-end (warm) | 58.44 s | 58.44 s | 58.44 s | 58.44 s | 1 |

**Throughput:** 2.177 DiT steps/s
...
- **weights load total:** 5.2 min (310 s) of 12.9 min (771 s) wall
...
| note | saved ltx_2_tp4tcod03_out.mp4 |
...
- per-step = 459.4 ms/DiT-step (median 458.8, p90 460.1, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronLTX2Application.forward_dit, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 33s; output finite=True.
...
- e2e_cold = 771 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 58 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 771 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4 + TeaCache online-delta adaptive (--teacache-online-delta 0.3; alpha sweep); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF) |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model ltx_2 --config tp4tcod03    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model ltx_2 --config tp4tcod03    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model ltx_2 --config tp4tcod03   # diffusers CUDA reference adapter
```
