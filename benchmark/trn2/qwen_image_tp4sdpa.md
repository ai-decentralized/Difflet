...
**Timestamp:** 2026-09-12 23:01 UTC

> Best-performing configuration: tp=4, --attention-impl sdpa (PyTorch SDPA via XLA instead of attention_cte); tp=4, bf16, joint attention via attention_cte
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 17.8 min (1068 s) |
| **e2e generate — cold start** (page cache dropped) | **8.4 min (503 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 7.5 min (447 s) |
| **e2e generate — warm cache** | **69.11 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 31.55 s |

> Cold vs warm: **8.4 min (503 s) → 69.11 s** (7.3× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 788.8 ms | 788.6 ms | 789.1 ms | 787.8 ms | 19 |
| end-to-end (warm) | 69.11 s | 69.11 s | 69.11 s | 69.11 s | 1 |
...
|---|---:|---:|---:|---:|---:|---:|
| text_encoder | 177.0 ms | 3.73 s | 60.41 s | 7.08 s | 32.99 s | **104.39 s** |
| transformer | 86.14 s | 6.71 s | 35.85 s | 6.0 ms | 2.7 min (162 s) | **4.8 min (290 s)** |
| vae_decoder | 339.0 ms | 224.0 ms | 6.7 min (402 s) | 0.0 ms | 9.70 s | **6.9 min (413 s)** |
| **Σ component builds** | | | | | | **13.5 min (807 s)** |

> The headline **compile = 17.8 min (1068 s)** is the full `difflet compile` wall; the **Σ component builds = 13.5 min (807 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
| transformer (denoise loop) | — | 5.3 min (315 s) |
| vae_decoder | — | 10.65 s |
| **weights load total** | 0.0 ms | **7.5 min (447 s)** |

- **weights load total:** 7.5 min (447 s) of 8.4 min (503 s) wall
- **compute + overhead (residual):** 56.11 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved qwen_image_tp4sdpa_out.png |
...
- per-step = 788.8 ms/DiT-step (median 788.6, p90 789.1, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronQwenImageTransformerApplication.__call__, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 33s; output finite=True.
- compile-only run: e2e/per-step come from cold_warm_e2e / step_realloop
- e2e_cold = 503 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 69 s (n=1, warm OS page cache from the immediately-preceding cold run; same session as the 503 s cold start). difflet reloads weights every process, so warm = warm disk cache -> faster load, not a resident model.
...
| best-perf knobs | tp=4, --attention-impl sdpa (PyTorch SDPA via XLA instead of attention_cte); tp=4, bf16, joint attention via attention_cte |
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model qwen_image --config tp4sdpa    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model qwen_image --config tp4sdpa    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model qwen_image --config tp4sdpa   # diffusers CUDA reference adapter
```
