...
**Timestamp:** 2026-06-27 04:49 UTC
...
| **e2e generate — cold start** (page cache dropped) | **30.43 s** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 4.33 s |
| **e2e generate — warm cache** | **27.78 s** |
| peak device memory | 59.5 GB |

> Cold vs warm: **30.43 s → 27.78 s** (1.1× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 874.5 ms | 874.8 ms | 875.2 ms | 872.6 ms | 19 |
| end-to-end (warm) | 27.78 s | 27.78 s | 27.78 s | 27.78 s | 1 |

**Throughput:** 1.144 steps/s
...
|---|---:|---:|
| pipeline load (from_pretrained → device) | — | 4.33 s |
| **weights load total** | — | **4.33 s** |

- **weights load total:** 4.33 s of 30.43 s wall
- **compute + overhead (residual):** 26.10 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| field | value |
|---|---|
| shape | [1, 61, 3, 320, 512] |
| dtype | torch.float32 |
| finite (no NaN/Inf) | True |
| value range | [0.0000, 1.0000] (mean 0.5506, std 0.2896) |
