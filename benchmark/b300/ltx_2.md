...
**Timestamp:** 2026-06-27 04:32 UTC
...
| **e2e generate — cold start** (page cache dropped) | **12.70 s** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 7.40 s |
| **e2e generate — warm cache** | **12.70 s** |
| peak device memory | 76.7 GB |

> Cold vs warm: **12.70 s → 12.70 s** (1.0× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 159.5 ms | 159.4 ms | 159.6 ms | 159.4 ms | 19 |
| end-to-end (warm) | 12.70 s | 12.70 s | 12.70 s | 12.70 s | 1 |

**Throughput:** 6.268 steps/s
...
|---|---:|---:|
| pipeline load (from_pretrained → device) | — | 7.40 s |
| **weights load total** | — | **7.40 s** |

- **weights load total:** 7.40 s of 12.70 s wall
- **compute + overhead (residual):** 5.31 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| value range | [0.0234, 1.0000] (mean 0.5950, std 0.2545) |
