...
| **e2e generate — warm cache** | **2.2 min (130 s)** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 35.57 s |

> Cold vs warm: **8.7 min (521 s) → 2.2 min (130 s)** (4.0× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.
...
| end-to-end (warm) | 2.2 min (130 s) | 2.1 min (128 s) | 2.2 min (134 s) | 2.1 min (126 s) | 3 |
...
- e2e_warm = 130 s (n=3; reported after 1 discarded cache-warming run(s) so the OS page cache is warm). The difflet CLI reloads weights every process, so 'warm' = warm disk cache -> faster load, not a resident model; cf. e2e cold and the load/compute breakdown.
