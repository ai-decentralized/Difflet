...
| **e2e generate — warm cache** | **3.2 min (189 s)** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 47.02 s |

> Cold vs warm: **14.3 min (859 s) → 3.2 min (189 s)** (4.5× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.
...
| end-to-end (warm) | 3.2 min (189 s) | 3.2 min (189 s) | 3.2 min (189 s) | 3.2 min (189 s) | 1 |
...
- e2e_warm = 189 s (n=1; reported after 2 discarded cache-warming run(s) so the OS page cache is warm). The difflet CLI reloads weights every process, so 'warm' = warm disk cache -> faster load, not a resident model; cf. e2e cold and the load/compute breakdown.
