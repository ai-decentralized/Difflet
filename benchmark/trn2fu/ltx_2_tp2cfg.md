...
**Timestamp:** 2026-09-12 18:33 UTC

> Best-performing configuration: tp=2 x CFG-parallel (uncond/cond on separate core pairs); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF)
...
| phase | time |
|---|---|
| compile (AOT, one-time) | 24.9 min (1495 s) |
| **e2e generate — cold start** (page cache dropped) | **17.9 min (1077 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 9.9 min (596 s) |
| **e2e generate — warm cache** | **8.5 min (510 s)** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 41.32 s |

> Cold vs warm: **17.9 min (1077 s) → 8.5 min (510 s)** (2.1× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 779.7 ms | 779.3 ms | 780.9 ms | 778.4 ms | 19 |
| end-to-end (warm) | 8.5 min (510 s) | 8.5 min (510 s) | 8.5 min (510 s) | 8.5 min (510 s) | 1 |

**Throughput:** 1.283 DiT steps/s
...
|---|---:|---:|---:|---:|---:|---:|
| transformer | 83.83 s | 29.09 s | 86.76 s | 12.0 ms | 8.7 min (520 s) | **12.0 min (720 s)** |
| **Σ component builds** | | | | | | **12.0 min (720 s)** |

> The headline **compile = 24.9 min (1495 s)** is the full `difflet compile` wall; the **Σ component builds = 12.0 min (720 s)** above is only the neuronx-cc build sub-phase. The difference is one-time host model load + HLO trace + weight shard/save before/around the builds (largest for big multi-encoder pipelines).
...
|---|---:|---:|
| transformer (denoise loop) [Neuron] | — | 9.9 min (596 s) |
| **weights load total** | 0.0 ms | **9.9 min (596 s)** |

- **weights load total:** 9.9 min (596 s) of 17.9 min (1077 s) wall
- **compute + overhead (residual):** 8.0 min (481 s) = text-encode + denoise loop + VAE decode + process/runtime startup
...
| note | saved ltx_2_tp2cfg_out.mp4 |
...
- per-step = 779.7 ms/DiT-step (median 779.3, p90 780.9, n=19) — measured the SAME way as H100: inter-step deltas of a real 20-step generate (wrapping NeuronLTX2Application.forward_dit, synced, step 0 excluded), NOT the old isolated synthetic-input timer. 20 DiT calls timed; warm generate 52s; output finite=True.
...
- e2e_cold = 1077 s — TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 510 s (n=1; reported after 2 discarded cache-warming run(s) so the OS page cache is warm). The difflet CLI reloads weights every process, so 'warm' = warm disk cache -> faster load, not a resident model; cf. e2e cold and the load/compute breakdown.
- resident generate = 52.4 s (generate 3 of 3 in one process, model loaded once; walls [77.5, 52.7, 52.4]) -- the serving steady state; per-step above is from the last generate.
...
| best-perf knobs | tp=2 x CFG-parallel (uncond/cond on separate core pairs); tp=4, bf16, TP-sharded transformer + attention_cte self-attn, guidance=1.0 (batch-1 NEFF) |
...
difflet compile  --model-id Lightricks/LTX-2 --revision 47da56e2ad66ce4125a9922b4a8826bf407f9d0a \
    --tp-degree 2 --cp-degree 1 --cfg-parallel --height 480 --width 704 --num-frames 49
...
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.cold_warm_e2e --model ltx_2 --config tp2cfg    # true cold + warm e2e
DIFFLET_BENCH_DEVICE=trn2 \
    python -m benchmark.step_latency  --model ltx_2 --config tp2cfg    # warm per-step

# other backends (H100/B300) reproduce the SAME model+config via the generic runner:
#   python -m benchmark.bench --backend cuda --model ltx_2 --config tp2cfg   # diffusers CUDA reference adapter
```
