...
**Timestamp:** 2026-09-12 06:18 UTC
...
| **e2e generate — cold start** (page cache dropped) | **3.3 min (197 s)** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 81.47 s |
| **e2e generate — warm cache** | **92.86 s** |
| &nbsp;&nbsp;↳ of which weights load (from page cache) | 5.86 s |
| **request on the resident process** (weights already on the device, n=2) | **33.12 s** |
...
> Cold vs warm: **3.3 min (197 s) → 92.86 s** (2.1× faster warm). e2e is load-dominated; the gap is the one-time cold disk read of the weights (warm = weights already in the OS page cache). The stable compute metric is the per-step latency below.

## Latency distribution

| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 612.5 ms | 611.8 ms | 617.2 ms | 605.0 ms | 38 |
| end-to-end (warm, fresh process) | 92.86 s | 92.77 s | 93.27 s | 92.52 s | 3 |
| request (resident process) | 33.12 s | 33.12 s | 33.18 s | 33.05 s | 2 |

**Throughput:** 1.633 steps/s
...
|---|---|
| throughput (denoise wall clock / steps) | 612.2 ms |
| enqueue rate (unsynced deltas — **not device time**) | 608.8 ms |
...
| metric | mean | median | p90 | min | n |
|---|---|---|---|---|---|
| per denoise step (natural) | 609.6 ms | 609.6 ms | 609.8 ms | 609.5 ms | 2 |
| end-to-end warm (natural) | 32.98 s | 32.98 s | 33.02 s | 32.94 s | 2 |
...
|---|---:|---:|
| transformer (DiT -> chips) + text_encoder (umT5, host fp32, ordinal 0) | — | 79.70 s |
| vae (host fp32, primary replica) | — | 1.77 s |
| **weights load total** | 0.0 ms | **81.47 s** |

- **weights load total:** 81.47 s of 3.3 min (197 s) wall
- **compute + overhead (residual):** 115.26 s = text-encode + denoise loop + VAE decode + process/runtime startup
...
| stage | weight load |
|---|---:|
| transformer (DiT -> chips) + text_encoder (umT5, host fp32, ordinal 0) | 4.47 s |
| vae (host fp32, primary replica) | 1.39 s |
| **weights load total** | **5.86 s** |

- **weights load total:** 5.86 s of 92.77 s wall
- **compute + overhead (residual):** 86.91 s
...
| value range | [-1.0000, 1.0000] (mean -0.3427, std 0.4460) |
| note | decoded video (BCTHW, minus_one_to_one); saved wan_2_1_warm2.mp4, wan_2_1_warm2_frames_0_4_8.png |
...
- e2e_cold = 197 s -- TRUE cold start (OS page cache dropped before the run), so the weight load is a real cold disk read.
- e2e_warm = 93 s (n=3; reported after 1 discarded cache-warming run(s) so the OS page cache is warm). Every run is a fresh process that reloads the weights, so 'warm' = warm disk cache -> faster load, not a resident model.
...
DIFFLET_BACKEND=tpu difflet serve --model-id Wan-AI/Wan2.1-T2V-14B-Diffusers --revision 38ec498cb3208fb688890f8cc7e94ede2cbd7f68 \
