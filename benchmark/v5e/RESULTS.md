...
HunyuanVideo and LTX-2 were ported on 2026-09-11/12 (`docs/plans/2026-09-11-tpu-port-hunyuan-ltx2-flux.md`).
...
| device | Qwen-Image | Wan 2.2 A14B | Wan 2.1 14B | HunyuanVideo | LTX-2 |
|---|---|---|---|---|---|
...
Per DiT step the v5e is 1.1× (Qwen, Wan) to 3.3× (LTX-2) slower than trn2's 4 cores; per
request it is faster wherever the decode is on the chip (Qwen, LTX-2) and slower where it is
not (the video VAEs on the host: Wan's 24 s, HunyuanVideo's 165 s). Putting those two VAEs on
...
model** (Qwen 7.60 s, Wan 9.10 s, Hunyuan 15.07 s, LTX-2 DiT 22 s); online-delta is a net loss on
