...
**Timestamp:** 2026-06-26 19:24 UTC
...
| **e2e generate — cold start** (page cache dropped) | **39.87 s** |
| &nbsp;&nbsp;↳ of which weights load (cold disk read) | 25.17 s |
...
|---|---|---|---|---|---|
| per denoise step (transformer fwd) | 564.4 ms | 563.2 ms | 567.1 ms | 551.9 ms | 19 |

**Throughput:** 1.772 steps/s
...
|---|---:|---:|
| pipeline load (from_pretrained → device) | — | 25.17 s |
| **weights load total** | — | **25.17 s** |

- **weights load total:** 25.17 s of 39.87 s wall
- **compute + overhead (residual):** 14.70 s = text-encode + denoise loop + VAE decode + process/runtime startup
