...
PROMPT="${DIFFLET_WAN_PROMPT:-a cat walking in a garden}"
STEPS="${DIFFLET_WAN_STEPS:-50}"
...
python examples/wan_example.py "${COMMON[@]}" \
  --save-latents "${ROOT}/.difflet-cache/wan_ab_baseline.pt" \
  --compiled-dir "${ROOT}/.difflet-cache/wan_ab_run_baseline"
echo "[wan-ab] baseline exit=$?"

echo "===== [wan-ab] TEACACHE (fixed-cadence) ====="
python examples/wan_example.py "${COMMON[@]}" \
  --teacache-calibration "${CALIB}" \
  --save-latents "${ROOT}/.difflet-cache/wan_ab_teacache.pt" \
  --compiled-dir "${ROOT}/.difflet-cache/wan_ab_run_teacache"
echo "[wan-ab] teacache exit=$?"

echo "===== [wan-ab] COMPARE ====="
python - <<'PY'
import torch, torch.nn.functional as F
b = torch.load('/home/ubuntu/difflet/.difflet-cache/wan_ab_baseline.pt', map_location='cpu').float().flatten()
t = torch.load('/home/ubuntu/difflet/.difflet-cache/wan_ab_teacache.pt', map_location='cpu').float().flatten()
