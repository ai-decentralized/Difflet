#!/usr/bin/env bash
# On-device Wan Megatron-SP vs dense-TP numerical parity gate (tp=2, no CP).
#
# Runs the Wan DiT backbone twice on a fixed-seed input — once dense (plain TP),
...
# latents. SP is mathematically lossless vs dense TP, so cosine should be ~1.0.
#
...
#
#   bash scripts/wan_sp_parity_smoke.sh
#
# Tunables (env): DIFFLET_WAN_MODEL, DIFFLET_WAN_TP_DEGREE (2),
...
WORKDIR="${DIFFLET_WAN_SP_PARITY_WORKDIR:-/tmp/wan_sp_parity}"
mkdir -p "${WORKDIR}"

cd "${ROOT}"

echo "[sp-parity] === dense ==="
"${PYTHON_BIN}" scripts/wan_sp_parity_smoke.py --mode dense --out "${WORKDIR}/out_dense.pt"

echo "[sp-parity] === sp ==="
"${PYTHON_BIN}" scripts/wan_sp_parity_smoke.py --mode sp --out "${WORKDIR}/out_sp.pt"
