"""Authoritative HunyuanVideo DiT trajectory parity: Nova Trainium vs HF diffusers.
...
Env: HY_SOURCE, HY_COMPILED, NOVA_ATTN_DROP_MASK (match the artifact).
"""
...
SOURCE = Path(os.environ.get("HY_SOURCE", f"{ROOT}/.nova-cache/hunyuan_n4_20d40s2r/source"))
COMPILED = Path(os.environ.get("HY_COMPILED", f"{ROOT}/.nova-cache/hunyuan_sdpa_20d40s2r/compiled"))
BUNDLE = f"{ROOT}/.nova-cache/hunyuan_dit_inputs/cat_walking_4step.safetensors"
DTYPE = torch.bfloat16  # Nova device dtype (fixed)
...
def run_nova():
    ...
    for i, t in enumerate(traj):
        print(f"[nova] step{i} std={t.std():.4f}", flush=True)
    return traj
...
nova = run_nova()
print("[parity] nova trajectory done; running HF CPU reference (slow) ...", flush=True)
hf = run_hf()

print("\n=== per-step trajectory cosine (Nova vs HF) ===", flush=True)
cosines = []
for i, (a, b) in enumerate(zip(nova, hf)):
