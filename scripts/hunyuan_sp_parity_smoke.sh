#!/usr/bin/env bash
# On-device HunyuanVideo Megatron-SP vs dense-TP numerical parity gate (tp=2, no CP).
#
# Runs the HunyuanVideo DiT backbone twice on a fixed-seed input — once dense
# (plain TP), once with Megatron-style sequence parallelism — each in its OWN
# process (NxD parallel_state initializes once per process), then compares the two
# output latents. SP is mathematically lossless vs dense TP, so cosine should be
# ~1.0.
#
...
#
#   bash scripts/hunyuan_sp_parity_smoke.sh
#
# Tunables (env): DIFFLET_HUNYUAN_MODEL, DIFFLET_HUNYUAN_TP_DEGREE (2),
#   DIFFLET_HUNYUAN_LAYERS (2), DIFFLET_HUNYUAN_SINGLE_LAYERS (2),
#   DIFFLET_HUNYUAN_HEIGHT (256), DIFFLET_HUNYUAN_WIDTH (256),
#   DIFFLET_HUNYUAN_NUM_FRAMES (5), DIFFLET_HUNYUAN_SP_PARITY_COSINE_MIN (0.999).
set -euo pipefail
...
COSINE_MIN="${DIFFLET_HUNYUAN_SP_PARITY_COSINE_MIN:-0.999}"
WORKDIR="${DIFFLET_HUNYUAN_SP_PARITY_WORKDIR:-/tmp/hunyuan_sp_parity}"
mkdir -p "${WORKDIR}"

cd "${ROOT}"

echo "[sp-parity] === dense ==="
"${PYTHON_BIN}" scripts/hunyuan_sp_parity_smoke.py --mode dense --out "${WORKDIR}/out_dense.pt"

echo "[sp-parity] === sp ==="
"${PYTHON_BIN}" scripts/hunyuan_sp_parity_smoke.py --mode sp --out "${WORKDIR}/out_sp.pt"
