#!/usr/bin/env bash
# On-device Qwen-Image ring-vs-gather-KV numerical parity gate (tp=2/cp=2).
#
# Runs the Qwen-Image DiT backbone twice on a fixed-seed input — once with
...
# packed-latent tensors. Ring is numerically lossless vs gather-KV, so cosine
# should be ~1.0.
#
...
#
#   bash scripts/qwen_ring_parity_smoke.sh
#
# Tunables (env):
#   DIFFLET_QWEN_MODEL              (Qwen/Qwen-Image)
#   DIFFLET_QWEN_TP_DEGREE          (2)
#   DIFFLET_QWEN_CP_DEGREE          (2)
#   DIFFLET_QWEN_LAYERS             (2)
#   DIFFLET_QWEN_HEIGHT             (256)
#   DIFFLET_QWEN_WIDTH              (256)
#   DIFFLET_QWEN_TEXT_SEQ_LEN       (256)
#   DIFFLET_QWEN_PARITY_COSINE_MIN  (0.999)
#
# NOTE: device parity is DEFERRED — Qwen-Image transformer weights are not
# cached on the compile box and the disk cannot fit them. This script is
# committed unrun. Run it on a box with the real weights and Trainium2 hardware.
...
TP_DEGREE="${DIFFLET_QWEN_TP_DEGREE:-2}"
CP_DEGREE="${DIFFLET_QWEN_CP_DEGREE:-2}"
...
WORKDIR="${DIFFLET_QWEN_PARITY_WORKDIR:-/tmp/qwen_ring_parity}"
mkdir -p "${WORKDIR}"

cd "${ROOT}"

echo "[parity] === gather_kv ==="
"${PYTHON_BIN}" scripts/qwen_ring_parity_smoke.py --mode gather_kv --out "${WORKDIR}/out_gather_kv.pt"

echo "[parity] === ring ==="
"${PYTHON_BIN}" scripts/qwen_ring_parity_smoke.py --mode ring --out "${WORKDIR}/out_ring.pt"
