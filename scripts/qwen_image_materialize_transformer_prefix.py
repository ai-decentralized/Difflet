#!/usr/bin/env python3
"""Materialize a Qwen-Image transformer snapshot with the first N blocks.

This is a debugging helper for parity bisection. It keeps the non-block
weights (input/time/final projections) and copies ``transformer_blocks.0`` …
``transformer_blocks.N-1`` from a full Qwen-Image transformer snapshot.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source-dir", required=True, help="Parent dir containing transformer/")
...
def main() -> int:
    ...
    for shard in _iter_safetensor_shards(source_transformer):
        print(f"[qwen-prefix] scan {shard.name}", flush=True)
