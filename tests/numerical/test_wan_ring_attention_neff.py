"""Wan ring-attention vs gather-KV parity gate (device-only).

Skipped by default — requires a multi-core CP launch (cp_degree > 1) on
Trainium2. Set DIFFLET_RUN_WAN_RING_NEFF=1 to enable.

The real work lives in ``scripts/wan_ring_parity_smoke.sh``: it runs the Wan DiT
...
Verified on trn2.3xlarge (4 cores, LNC2) at tp=2/cp=2: cosine 0.999968.
"""
...
def test_wan_ring_matches_gather_kv_neff():
    ...
    driver = repo_root / "scripts" / "wan_ring_parity_smoke.sh"
    ...
    # (override via DIFFLET_WAN_PARITY_COSINE_MIN). It exits non-zero on mismatch or
    ...
    assert result.returncode == 0, (
        f"wan_ring_parity_smoke.sh failed (rc={result.returncode}).\n"
        f"--- stdout tail ---\n{result.stdout[-3000:]}\n"
        f"--- stderr tail ---\n{result.stderr[-2000:]}"
    )
