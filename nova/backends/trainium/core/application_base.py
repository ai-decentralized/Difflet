...
from nova.backends.trainium.core.model_wrapper import (
...
# Nova fork: LLM-LoRA serving is not on the diffusion path. Stubbed to keep
# the symbol available for type/isinstance checks without depending on NxDI
# at runtime. Re-enable when diffusion LoRA support lands (M5).
LoraModelManager = None  # type: ignore[assignment]
# Original: from neuronx_distributed_inference.modules.lora_serving import LoraModelManager
from nova.backends.trainium.utils.runtime_env import set_env_vars
from nova.backends.trainium.utils.compile_env import set_compile_env_vars
from nova.backends.trainium.utils.snapshot import (
    ScriptModuleWrapper,
