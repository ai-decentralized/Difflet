...
- Flux is a Nova fork (NeuronFluxTransformer2DModel with NXD parallel layers),
  not a diffusers wrapper, so the trace module wraps the Nova transformer.
...
class _FluxTransformerTraceModule(nn.Module):
    """Wraps the Nova Flux transformer; exposes the block-0 modulated input."""
