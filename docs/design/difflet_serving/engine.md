...
When S3 is configured, artifact upload and URL generation happen after the engine
...
S3 variables, the handler returns a Base64 data URL and does not use an artifact
backend. A partial S3 configuration returns `artifact_store_unavailable` during
startup. Unexpected SDK/backend failures retain full details only in protected
logs and return fixed `internal_error`; a hung S3 upload or presign must not hold
