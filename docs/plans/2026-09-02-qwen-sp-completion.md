...
|---|---:|---:|---:|---:|---:|---:|---|
| tp4 | 1204.9 | 447.7 | 31.1 | 494.4 | 60.8 ±1.0 | 421.7 ±0.8 | ✅ |
| **tp4sp** | 1464.4 | 443.9 | 31.2 | 490.5 | **59.6 ±0.9** | **367.7 ±0.7** | ✅ |

**结论：Qwen-Image 上 SP 首次真正转正 —— 单步 -12.8%（367.7 vs 421.7ms），热
e2e -2.0%。** 与 8/28 跨模型结论（FLUX tp4sp +3.1%、Wan +5.3~10.5% 均为开销）
对照，解释也自洽：Qwen 是双流 MMDiT（image/text 两条残差流都被 SP 分片，
norm/modulation/residual/gate 的算子与激活节省双份），且 1024² 的序列更长，
g/ḡ 通信占比更低。通用规律的修正版：SP 的收益随"被分片的残差流条数 ×
序列长度"增长，长序列双流模型上为正，短序列单流模型上为负。

其它备注：

- tp4sp 编译宿主内存峰值 ~125GB anon RSS > 124GiB 物理内存，首次编译被 OOM
  kill；加 200GB NVMe swapfile 后完成（trace 峰值 ~31GB 落 swap，总时长
  1464s vs dense 1205s，+21.6%）。**tp4sp 编译需要 swap（或 >126GB RAM）**
  —— 这可能正是 8/28 旧机未测成的原因之一。
- 与旧机交叉参考：旧 tp4 热 e2e 56.4s / 本机 60.8s（+7.8%，工具链 cc 2.24→
  2.27 等），再次印证同机成对测量的必要性。
- step 阶段 n=19：guidance=4.0 的蒸馏模型每步单前向，20 步弃 step0 → 19。
  std 均 <1ms，测量极稳定。
